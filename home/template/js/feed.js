...
$(document).ready(function(){
  ...
  var status=$.query.get('status');
	$.ajax({
			dataType: "jsonp",
			url: "http://v5.home3d.cn/home/capi/space.php?do="+idtype+"&uid="+uid+"&status="+status+"&page=1&perpage=5",
...
if(""+idtype+""=="auction"){
          data=data.data.auction;
           if (data.length<=0){
            $(".more_button").appendto("<div class='more_button'>亲，没有了哦！</div>");
          }
}
if(""+idtype+""=="seckill"){
          data=data.data.seckill;
           if (data.length<=0){
            $(".more_button").appendto("<div class='more_button'>亲，没有了哦！</div>");
          }
}
if(""+idtype+""=="stratch"){
          data=data.data.stratch;
           if (data.length<=0){
            $(".more_button").appendto("<div class='more_button'>亲，没有了哦！</div>");
          }
}
if(""+idtype+""=="debate"){
          data=data.data.debate;
           if (data.length<=0){
            $(".more_button").appendto("<div class='more_button'>亲，没有了哦！</div>");
          }
}
if(""+idtype+""=="guess"){
          data=data.data.guess;
           if (data.length<=0){
            $(".more_button").appendto("<div class='more_button'>亲，没有了哦！</div>");
          }
}
...
//转发功能
  ...
  //分页
   function getComment(status,idtype, uid, page, perpage){
   $("#morebtn .ui-btn-text").html("正在加载...");
   $("#morebtn").addClass('ui-disabled');
  $.ajax({
      dataType: "jsonp",
      url: "http://v5.home3d.cn/home/capi/space.php?do="+idtype+"&status="+status+"&uid=" + uid + "&page=" + page + "&perpage=" + perpage,
...
if(""+idtype+""=="event"){
         ...
         // oid2=data.quiz.options[1].oid;
          ...
          if(data[i].image1url){
            ...
            data[i].image1url ="../"+data[i].imageurl;
          }
        }
          //data.user = getUser(data.uid, auth);
          //data.idtype = "photoid";
