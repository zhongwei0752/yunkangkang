...
//Ìí¼Ó²©¿Í
function product_post($POST, $olds=array()) {
		...
		if(!empty($POST['classid']) && substr($POST['classid'], 0, 4) == 'new:') {
			...
			if(empty($classname)) {
				...
				if(empty($classid)) {
					...
					$classid = inserttable('classproduct', $setarr, 1);
				}
			}
		} else {
	...
	if($POST['tag'] != $oldtagstr) {
		if(!empty($olds['tag'])) {
			//ÏÈ°ÑÒÔÇ°µÄ¸øÇåÀíµô
			$oldtags = array();
			$query = $_SGLOBAL['db']->query("SELECT tagid, productid FROM ".tname('tagproduct')." WHERE productid='$productid'");
			while ($value = $_SGLOBAL['db']->fetch_array($query)) {
				$oldtags[] = $value['tagid'];
			}
			if($oldtags) {
				$_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET productnum=productnum-1 WHERE tagid IN (".simplode($oldtags).")");
...
if($olds) {
	...
	include("./source/upload.class.php");
  	$image= new upload;
  	$image->upload_file($productid,"product");

	//²úÉúfeed
	if($POST['makefeed']) {
		include_once(S_ROOT.'./source/function_feed.php');
		feed_publish($productid, 'productid', $olds?0:1);
	}
