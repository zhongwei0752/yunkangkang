...
/*
	...
	$Id: function_branch.php 13245 2009-08-25 02:01:40Z liguode $
*/
...
//添加博客
function branch_post($POST, $olds=array()) {
	...
	if(empty($olds['classid']) || $POST['classid'] != $olds['classid']) {
		if(!empty($POST['classid']) && substr($POST['classid'], 0, 4) == 'new:') {
			...
			if(empty($classname)) {
				...
				$classid = getcount('classbranch', array('classname'=>$classname, 'uid'=>$_SGLOBAL['supe_uid']), 'classid');
				if(empty($classid)) {
					$setarr = array(
						'classname' => $classname,
					...
					$classid = inserttable('classbranch', $setarr, 1);
				}
			}
		} else {
	...
	if(!empty($POST['picids'])) {
		...
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			if(empty($titlepic) && $value['thumb']) {
				...
				$brancharr['picflag'] = $value['remote']?2:1;
			}
	...
	$brancharr['pic'] = $titlepic;
	
	//热度
	if(checkperm('managebranch')) {
		$brancharr['hot'] = intval($POST['hot']);
	}
	
	if($olds['branchid']) {
		//更新
		$branchid = $olds['branchid'];
		updatetable('branch', $brancharr, array('branchid'=>$branchid));
		
		$fuids = array();
		
		$brancharr['uid'] = $olds['uid'];
		$brancharr['username'] = $olds['username'];
	} else {
		//参与热闹
		$brancharr['topicid'] = topic_check($POST['topicid'], 'branch');

		$brancharr['uid'] = $_SGLOBAL['supe_uid'];
		$brancharr['username'] = $_SGLOBAL['supe_username'];
		$brancharr['dateline'] = empty($POST['dateline'])?$_SGLOBAL['timestamp']:$POST['dateline'];
		$branchid = inserttable('branch', $brancharr, 1);
	}
	...
	if($POST['tag'] != $oldtagstr) {
		if(!empty($olds['tag'])) {
			//先把以前的给清理掉
			$oldtags = array();
			$query = $_SGLOBAL['db']->query("SELECT tagid, branchid FROM ".tname('tagbranch')." WHERE branchid='$branchid'");
			while ($value = $_SGLOBAL['db']->fetch_array($query)) {
				$oldtags[] = $value['tagid'];
			}
			if($oldtags) {
				$_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET branchnum=branchnum-1 WHERE tagid IN (".simplode($oldtags).")");
				$_SGLOBAL['db']->query("DELETE FROM ".tname('tagbranch')." WHERE branchid='$branchid'");
			}
		}
		$tagarr = tag_batch($branchid, $POST['tag']);
		//更新附表中的tag
	...
	if($olds) {
		//更新
		updatetable('branchfield', $fieldarr, array('branchid'=>$branchid));
	} else {
		$fieldarr['branchid'] = $branchid;
		$fieldarr['uid'] = $brancharr['uid'];
		inserttable('branchfield', $fieldarr);
	}
	...
	if($isself) {
		if($olds) {
			...
			if(empty($space['branchnum'])) {
				$space['branchnum'] = getcount('branch', array('uid'=>$space['uid']));
				$branchnumsql = "branchnum=".$space['branchnum'];
			} else {
				$branchnumsql = 'branchnum=branchnum+1';
			}
			//积分
			$reward = getreward('publishbranch', 0);
			$_SGLOBAL['db']->query("UPDATE ".tname('space')." SET {$branchnumsql}, lastpost='$_SGLOBAL[timestamp]', updatetime='$_SGLOBAL[timestamp]', credit=credit+$reward[credit], experience=experience+$reward[experience] WHERE uid='$_SGLOBAL[supe_uid]'");
	...
	include("./source/upload.class.php");
  	$image= new upload;
  	$image->upload_file($branchid,"branch");
	//产生feed
	if($POST['makefeed']) {
		include_once(S_ROOT.'./source/function_feed.php');
		feed_publish($branchid, 'branchid', $olds?0:1);
	}
	
	//热闹
	if(empty($olds) && $brancharr['topicid']) {
		topic_join($brancharr['topicid'], $_SGLOBAL['supe_uid'], $_SGLOBAL['supe_username']);
	}
...
//处理tag
function tag_batch($branchid, $tags) {
	global $_SGLOBAL;
	...
	if($updatetagids) $_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET branchnum=branchnum+1 WHERE tagid IN (".simplode($updatetagids).")");
	$tagids = array_keys($tagarr);
	$inserts = array();
	foreach ($tagids as $tagid) {
		$inserts[] = "('$tagid','$branchid')";
	}
	if($inserts) $_SGLOBAL['db']->query("REPLACE INTO ".tname('tagbranch')." (tagid,branchid) VALUES ".implode(',', $inserts));
...
//视频标签处理
function branch_bbcode($message) {
	$message = preg_replace("/\[flash\=?(media|real)*\](.+?)\[\/flash\]/ie", "branch_flash('\\2', '\\1')", $message);
	return $message;
}
//视频
function branch_flash($swf_url, $type='') {
	$width = '520';
	$height = '390';
	if ($type == 'media') {
