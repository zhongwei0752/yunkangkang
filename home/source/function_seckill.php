...
/*
	...
	$Id: function_seckill.php 13245 2009-08-25 02:01:40Z liguode $
*/
...
//Ìí¼Ó²©¿Í
function seckill_post($POST, $olds=array()) {
	...
	if(empty($olds['classid']) || $POST['classid'] != $olds['classid']) {
		if(!empty($POST['classid']) && substr($POST['classid'], 0, 4) == 'new:') {
			...
			if(empty($classname)) {
				...
				$classid = getcount('classseckill', array('classname'=>$classname, 'uid'=>$_SGLOBAL['supe_uid']), 'classid');
				if(empty($classid)) {
					$setarr = array(
						'classname' => $classname,
					...
					$classid = inserttable('classseckill', $setarr, 1);
				}
			}
		} else {
	...
	if($classid && empty($classname)) {
		//ÊÇ·ñÊÇ×Ô¼ºµÄ
		$classname = getcount('classseckill', array('classid'=>$classid, 'uid'=>$_SGLOBAL['supe_uid']), 'classname');
	...
	$seckillarr = array(
		...
		"starttime0" => sstrtotime($POST['starttime']),
		"endtime" => sstrtotime($POST['endtime']),
		'price'=> $POST['price'],
		'seckillpassword'=>$POST['seckillpassword'],
		'amount'=>$POST['amount'],
		'classid' => $classid,
		'friend' => $POST['friend'],
	...
	if(!empty($POST['picids'])) {
		...
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			if(empty($titlepic) && $value['thumb']) {
				...
				$seckillarr['picflag'] = $value['remote']?2:1;
			}
	...
	if(empty($titlepic)) {
		...
		$seckillarr['picflag'] = 0;
	}
	$seckillarr['pic'] = $titlepic;
	
	//ÈÈ¶È
	if(checkperm('manageseckill')) {
		$seckillarr['hot'] = intval($POST['hot']);
	}
	if($olds['seckillid']) {
		//¸üÐÂ
		$seckillid = $olds['seckillid'];
		updatetable('seckill', $seckillarr, array('seckillid'=>$seckillid));
		
		$fuids = array();
		
		$seckillarr['uid'] = $olds['uid'];
		$seckillarr['username'] = $olds['username'];
	} else {
		//²ÎÓëÈÈÄÖ
		$seckillarr['topicid'] = topic_check($POST['topicid'], 'seckill');

		$seckillarr['uid'] = $_SGLOBAL['supe_uid'];
		$seckillarr['username'] = $_SGLOBAL['supe_username'];
		$seckillarr['dateline'] = empty($POST['dateline'])?$_SGLOBAL['timestamp']:$POST['dateline'];
		$seckillid = inserttable('seckill', $seckillarr, 1);
	}
	
	$seckillarr['seckillid'] = $seckillid;
	...
	if($POST['tag'] != $oldtagstr) {
		if(!empty($olds['tag'])) {
			//ÏÈ°ÑÒÔÇ°µÄ¸øÇåÀíµô
			$oldtags = array();
			$query = $_SGLOBAL['db']->query("SELECT tagid, seckillid FROM ".tname('tagseckill')." WHERE seckillid='$seckillid'");
			while ($value = $_SGLOBAL['db']->fetch_array($query)) {
				$oldtags[] = $value['tagid'];
			}
			if($oldtags) {
				$_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET seckillnum=seckillnum-1 WHERE tagid IN (".simplode($oldtags).")");
				$_SGLOBAL['db']->query("DELETE FROM ".tname('tagseckill')." WHERE seckillid='$seckillid'");
			}
		}
		$tagarr = tag_batch($seckillid, $POST['tag']);
		//¸üÐÂ¸½±íÖÐµÄtag
	...
	if($olds) {
		//¸üÐÂ
		updatetable('seckillfield', $fieldarr, array('seckillid'=>$seckillid));
	} else {
		$fieldarr['seckillid'] = $seckillid;
		$fieldarr['uid'] = $seckillarr['uid'];
		inserttable('seckillfield', $fieldarr);
	}
	...
	if($isself) {
		if($olds) {
			...
			if(empty($space['seckillnum'])) {
				$space['seckillnum'] = getcount('seckill', array('uid'=>$space['uid']));
				$seckillnumsql = "seckillnum=".$space['seckillnum'];
			} else {
				$seckillnumsql = 'seckillnum=seckillnum+1';
			}
			//»ý·Ö
			$reward = getreward('publishseckill', 0);
			$_SGLOBAL['db']->query("UPDATE ".tname('space')." SET {$seckillnumsql}, lastpost='$_SGLOBAL[timestamp]', updatetime='$_SGLOBAL[timestamp]', credit=credit+$reward[credit], experience=experience+$reward[experience] WHERE uid='$_SGLOBAL[supe_uid]'");
	...
	include("./source/upload.class.php");
  	$image= new upload;
  	$image->upload_file($seckillid,"seckill");
	//²úÉúfeed
	if($POST['makefeed']) {
		include_once(S_ROOT.'./source/function_feed.php');
		feed_publish($seckillid, 'seckillid', $olds?0:1);
	}
	
	//ÈÈÄÖ
	if(empty($olds) && $seckillarr['topicid']) {
		topic_join($seckillarr['topicid'], $_SGLOBAL['supe_uid'], $_SGLOBAL['supe_username']);
	}
...
//´¦Àítag
function tag_batch($seckillid, $tags) {
	global $_SGLOBAL;
	...
	if($updatetagids) $_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET seckillnum=seckillnum+1 WHERE tagid IN (".simplode($updatetagids).")");
	$tagids = array_keys($tagarr);
	$inserts = array();
	foreach ($tagids as $tagid) {
		$inserts[] = "('$tagid','$seckillid')";
	}
	if($inserts) $_SGLOBAL['db']->query("REPLACE INTO ".tname('tagseckill')." (tagid,seckillid) VALUES ".implode(',', $inserts));
...
//ÊÓÆµ±êÇ©´¦Àí
function seckill_bbcode($message) {
	$message = preg_replace("/\[flash\=?(media|real)*\](.+?)\[\/flash\]/ie", "seckill_flash('\\2', '\\1')", $message);
	return $message;
}
//ÊÓÆµ
function seckill_flash($swf_url, $type='') {
	$width = '520';
	$height = '390';
	if ($type == 'media') {
