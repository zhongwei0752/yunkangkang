...
/*
	...
	$Id: function_development.php 13245 2009-08-25 02:01:40Z liguode $
*/
...
//Ìí¼Ó²©¿Í
function development_post($POST, $olds=array()) {
	...
	if(empty($olds['classid']) || $POST['classid'] != $olds['classid']) {
		if(!empty($POST['classid']) && substr($POST['classid'], 0, 4) == 'new:') {
			...
			if(empty($classname)) {
				...
				$classid = getcount('classdevelopment', array('classname'=>$classname, 'uid'=>$_SGLOBAL['supe_uid']), 'classid');
				if(empty($classid)) {
					$setarr = array(
						'classname' => $classname,
					...
					$classid = inserttable('classdevelopment', $setarr, 1);
				}
			}
		} else {
	...
	if($classid && empty($classname)) {
		//ÊÇ·ñÊÇ×Ô¼ºµÄ
		$classname = getcount('classdevelopment', array('classid'=>$classid, 'uid'=>$_SGLOBAL['supe_uid']), 'classname');
	...
	if(!empty($POST['picids'])) {
		...
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			if(empty($titlepic) && $value['thumb']) {
				...
				$developmentarr['picflag'] = $value['remote']?2:1;
			}
	...
	if(empty($titlepic)) {
		...
		$developmentarr['picflag'] = 0;
	}
	$developmentarr['pic'] = $titlepic;
	...
	if($olds['developmentid']) {
		...
		$developmentarr['username'] = $_SGLOBAL['supe_username'];
		$developmentarr['dateline'] = empty($POST['dateline'])?$_SGLOBAL['timestamp']:$POST['dateline'];
		$developmentid = inserttable('development', $developmentarr, 1);
	}
	...
	if($POST['tag'] != $oldtagstr) {
		if(!empty($olds['tag'])) {
			//ÏÈ°ÑÒÔÇ°µÄ¸øÇåÀíµô
			$oldtags = array();
			$query = $_SGLOBAL['db']->query("SELECT tagid, developmentid FROM ".tname('tagdevelopment')." WHERE developmentid='$developmentid'");
			while ($value = $_SGLOBAL['db']->fetch_array($query)) {
				$oldtags[] = $value['tagid'];
			}
			if($oldtags) {
				$_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET developmentnum=developmentnum-1 WHERE tagid IN (".simplode($oldtags).")");
				$_SGLOBAL['db']->query("DELETE FROM ".tname('tagdevelopment')." WHERE developmentid='$developmentid'");
			}
		}
		$tagarr = tag_batch($developmentid, $POST['tag']);
		//¸üÐÂ¸½±íÖÐµÄtag
	...
	if($isself) {
		if($olds) {
			...
			if(empty($space['developmentnum'])) {
				$space['developmentnum'] = getcount('development', array('uid'=>$space['uid']));
				$developmentnumsql = "developmentnum=".$space['developmentnum'];
			} else {
				$developmentnumsql = 'developmentnum=developmentnum+1';
			}
			//»ý·Ö
			$reward = getreward('publishdevelopment', 0);
			$_SGLOBAL['db']->query("UPDATE ".tname('space')." SET {$developmentnumsql}, lastpost='$_SGLOBAL[timestamp]', updatetime='$_SGLOBAL[timestamp]', credit=credit+$reward[credit], experience=experience+$reward[experience] WHERE uid='$_SGLOBAL[supe_uid]'");
			
			//Í³¼Æ
			updatestat('development');
		}
	}
	
	include("./source/upload.class.php");
  	$image= new upload;
  	$image->upload_file($developmentid,"development");
	//²úÉúfeed
	if($POST['makefeed']) {
		include_once(S_ROOT.'./source/function_feed.php');
		feed_publish($developmentid, 'developmentid', $olds?0:1);
	}
	
	//ÈÈÄÖ
	if(empty($olds) && $developmentarr['topicid']) {
		topic_join($developmentarr['topicid'], $_SGLOBAL['supe_uid'], $_SGLOBAL['supe_username']);
	}

	//½ÇÉ«ÇÐ»»
	if(!empty($__SGLOBAL)) $_SGLOBAL = $__SGLOBAL;

	return $developmentarr;
}

//´¦Àítag
function tag_batch($developmentid, $tags) {
	global $_SGLOBAL;
	...
	if($updatetagids) $_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET developmentnum=developmentnum+1 WHERE tagid IN (".simplode($updatetagids).")");
	$tagids = array_keys($tagarr);
	$inserts = array();
	foreach ($tagids as $tagid) {
		$inserts[] = "('$tagid','$developmentid')";
	}
	if($inserts) $_SGLOBAL['db']->query("REPLACE INTO ".tname('tagdevelopment')." (tagid,developmentid) VALUES ".implode(',', $inserts));
...
//ÊÓÆµ±êÇ©´¦Àí
function development_bbcode($message) {
	$message = preg_replace("/\[flash\=?(media|real)*\](.+?)\[\/flash\]/ie", "development_flash('\\2', '\\1')", $message);
	return $message;
}
//ÊÓÆµ
function development_flash($swf_url, $type='') {
	$width = '520';
	$height = '390';
	if ($type == 'media') {
