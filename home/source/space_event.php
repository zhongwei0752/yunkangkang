...
if($eventid){// ��ʾ�����
	...
	$event['replynum'] = $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('comment')." WHERE $csql id='$eventid' AND idtype='eventid'"),0);
	...
	if($event['public'] == 0 && $_SGLOBAL['supe_userevent']['status'] < 2 && !$allowmanage){
		...
		$value = $_SGLOBAL['db']->fetch_array($query);
		if(empty($value)){
			showmessage("event_not_public"); // ����һ��˽�ܻ����Ҫͨ��������ܲ鿴
		}
	}
	...
	if($view == "member"){
		...
		$count = $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT count(*) FROM ".tname("eventgo")." WHERE eid = '$eventid'"),0);
		$members = $fuids = array();
		if($count){
			$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname("eventgo")." WHERE eid = '$eventid' LIMIT $start, $perpage");
		...
		$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname("eventgo")." WHERE eid = '$eventid'");
			while($value = $_SGLOBAL['db']->fetch_array($query)){
