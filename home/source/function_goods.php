...
function goods_post($POST, $olds=array()) {
		...
		if(!empty($POST['classid']) && substr($POST['classid'], 0, 4) == 'new:') {
			...
			if(empty($classname)) {
				...
				if(empty($classid)) {
					...
					$classid = inserttable('classgoods', $setarr, 1);
				}
			}
		} else {
	...
	if($POST['push1']){
		$goodsarr['push']=$POST['push1'];
	}
	if($POST['push']){
		$goodsarr['push']=$POST['push'];
	}
	if($POST['fenlei']){
		$goodsarr['fenlei']=$POST['fenlei'];
	}
	//±êÌâÍ¼Æ¬
	$titlepic = '';
	...
	if($POST['makefeed']) {
		...
		feed_publish($goodsid, 'goodsid', $olds?0:1);
	}
