<?php
	...
	//代理人数确认
	$daili=getspace($_SGLOBAL['supe_uid']);
	if($daili['groupid']=='10'){
		$namestatus = $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('space')." s WHERE s.recomendman='$_SGLOBAL[supe_uid]' and s.namestatus='0'"), 0);	
	}
	
	//刚开通的默认值输入
	/*$query5 = $_SGLOBAL['db']->query("SELECT * FROM ".tname('appset')." a LEFT JOIN ".tname('menuset')." m ON a.num=m.menusetid WHERE a.uid='$space[uid]' and appstatus='1'");
	while($value5 = $_SGLOBAL['db']->fetch_array($query5)){
		$english=$value5['english'];
		if($english=='introduce'||$english=='product'||$english=='development'||$english=='branch'||$english=='industry'||$english=='job'||$english=='goods'||$english=='recommend'
			||$english=='auction'||$english=='event'||$english=='dial'||$english=='debate'||$english=='poll'){
		$query6 = $_SGLOBAL['db']->query("SELECT * FROM ".tname($english)." WHERE uid='$space[uid]'");
		$value6 = $_SGLOBAL['db']->fetch_array($query6);
		if(empty($value6)){
			$field=$english."field";
			$id=$english."id";
			inserttable("$english",array('uid'=>$space['uid'],'username'=>$space['name'],'subject'=>'测试标题','dateline'=>$_SGLOBAL['timestamp']));
			inserttable("$field",array("$id"=>$value6["$id"],'uid'=>$space['uid'],'message'=>"测试内容，左侧的图片请使用英文命名的jpg图片",'message1'=>"测试内容，左侧的图片请使用英文命名的jpg图片"));
			}
		}elseif($english=='stratch'){
			$query6 = $_SGLOBAL['db']->query("SELECT * FROM ".tname($english)." WHERE uid='$space[uid]'");
			$value6 = $_SGLOBAL['db']->fetch_array($query6);
			if(empty($value6)){*/
			//$field=$english."field";
			//$id=$english."id";
			//inserttable("$english",array('uid'=>$space['uid'],'username'=>$space['name'],'subject'=>'测试标题','dateline'=>$_SGLOBAL['timestamp'],'message'=>"测试内容，左侧的图片请使用英文命名的jpg图片",'message1'=>"测试内容，左侧的图片请使用英文命名的jpg图片"));
			//inserttable("$field",array("$id"=>$value6["$id"],'uid'=>$space['uid'],'message'=>"测试内容，左侧的图片请使用英文命名的jpg图片",'message1'=>"测试内容，左侧的图片请使用英文命名的jpg图片"));
			//}
		//}
	//}	
...
*/
	...
	$username=$_POST['username'];
	$password=$_POST['password'];
	$email = isemail($_POST['email']) ? $_POST['email'] :"623610577@qq.com";
	require_once S_ROOT.'./common.php';
    include_once( S_ROOT.'./wx/saetv2.ex.class.php' );
...
$myapp = $_SGLOBAL['db']->query("SELECT bf.*, b.* FROM ".tname('appset')." bf 
				...
				WHERE bf.uid='$_SGLOBAL[supe_uid]' and bf.appstatus='1' and b.english!='order'
				ORDER BY b.dateline ASC ");
		while ($myvalue = $_SGLOBAL['db']->fetch_array($myapp)) {
			if($myvalue['english']!='fenlei'){
