...
/*
	...
	$Id: function_explaining.php 13245 2009-08-25 02:01:40Z liguode $
*/
...
//Ìí¼Ó²©¿Í
function explaining_post($POST, $olds=array()) {
	...
	if(empty($olds['classid']) || $POST['classid'] != $olds['classid']) {
		if(!empty($POST['classid']) && substr($POST['classid'], 0, 4) == 'new:') {
			...
			if(empty($classname)) {
				...
				$classid = getcount('classexplaining', array('classname'=>$classname, 'uid'=>$_SGLOBAL['supe_uid']), 'classid');
				if(empty($classid)) {
					$setarr = array(
						'classname' => $classname,
					...
					$classid = inserttable('classexplaining', $setarr, 1);
				}
			}
		} else {
	...
	if($classid && empty($classname)) {
		//ÊÇ·ñÊÇ×Ô¼ºµÄ
		$classname = getcount('classexplaining', array('classid'=>$classid, 'uid'=>$_SGLOBAL['supe_uid']), 'classname');
	...
	if(!empty($POST['picids'])) {
		...
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			if(empty($titlepic) && $value['thumb']) {
				...
				$explainingarr['picflag'] = $value['remote']?2:1;
			}
	...
	if(empty($titlepic)) {
		...
		$explainingarr['picflag'] = 0;
	}
	$explainingarr['pic'] = $titlepic;
	
	//ÈÈ¶È
	if(checkperm('manageexplaining')) {
		$explainingarr['hot'] = intval($POST['hot']);
	}
	if($olds['explainingid']) {
		//¸üÐÂ
		$explainingid = $olds['explainingid'];
		updatetable('explaining', $explainingarr, array('explainingid'=>$explainingid));
		
		$fuids = array();
		
		$explainingarr['uid'] = $olds['uid'];
		$explainingarr['username'] = $olds['username'];
	} else {
		//²ÎÓëÈÈÄÖ
		$explainingarr['topicid'] = topic_check($POST['topicid'], 'explaining');

		$explainingarr['uid'] = $_SGLOBAL['supe_uid'];
		$explainingarr['username'] = $_SGLOBAL['supe_username'];
		$explainingarr['dateline'] = empty($POST['dateline'])?$_SGLOBAL['timestamp']:$POST['dateline'];
		$explainingid = inserttable('explaining', $explainingarr, 1);
	}
	
	$explainingarr['explainingid'] = $explainingid;
	...
	if($POST['tag'] != $oldtagstr) {
		if(!empty($olds['tag'])) {
			//ÏÈ°ÑÒÔÇ°µÄ¸øÇåÀíµô
			$oldtags = array();
			$query = $_SGLOBAL['db']->query("SELECT tagid, explainingid FROM ".tname('tagexplaining')." WHERE explainingid='$explainingid'");
			while ($value = $_SGLOBAL['db']->fetch_array($query)) {
				$oldtags[] = $value['tagid'];
			}
			if($oldtags) {
				$_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET explainingnum=explainingnum-1 WHERE tagid IN (".simplode($oldtags).")");
				$_SGLOBAL['db']->query("DELETE FROM ".tname('tagexplaining')." WHERE explainingid='$explainingid'");
			}
		}
		$tagarr = tag_batch($explainingid, $POST['tag']);
		//¸üÐÂ¸½±íÖÐµÄtag
	...
	if($olds) {
		//¸üÐÂ
		updatetable('explainingfield', $fieldarr, array('explainingid'=>$explainingid));
	} else {
		$fieldarr['explainingid'] = $explainingid;
		$fieldarr['uid'] = $explainingarr['uid'];
		inserttable('explainingfield', $fieldarr);
	}
	...
	if($isself) {
		if($olds) {
			...
			if(empty($space['explainingnum'])) {
				$space['explainingnum'] = getcount('explaining', array('uid'=>$space['uid']));
				$explainingnumsql = "explainingnum=".$space['explainingnum'];
			} else {
				$explainingnumsql = 'explainingnum=explainingnum+1';
			}
			//»ý·Ö
			$reward = getreward('publishexplaining', 0);
			$_SGLOBAL['db']->query("UPDATE ".tname('space')." SET {$explainingnumsql}, lastpost='$_SGLOBAL[timestamp]', updatetime='$_SGLOBAL[timestamp]', credit=credit+$reward[credit], experience=experience+$reward[experience] WHERE uid='$_SGLOBAL[supe_uid]'");
			
			//Í³¼Æ
			updatestat('explaining');
		}
	}
	include("./source/upload.class.php");
  	$image= new upload;
  	$image->upload_file($explainingid,"explaining");
	//²úÉúfeed
	if($POST['makefeed']) {
		include_once(S_ROOT.'./source/function_feed.php');
		feed_publish($explainingid, 'explainingid', $olds?0:1);
	}
	
	//ÈÈÄÖ
	if(empty($olds) && $explainingarr['topicid']) {
		topic_join($explainingarr['topicid'], $_SGLOBAL['supe_uid'], $_SGLOBAL['supe_username']);
	}

	//½ÇÉ«ÇÐ»»
	if(!empty($__SGLOBAL)) $_SGLOBAL = $__SGLOBAL;

	return $explainingarr;
}

//´¦Àítag
function tag_batch($explainingid, $tags) {
	global $_SGLOBAL;
	...
	if($updatetagids) $_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET explainingnum=explainingnum+1 WHERE tagid IN (".simplode($updatetagids).")");
	$tagids = array_keys($tagarr);
	$inserts = array();
	foreach ($tagids as $tagid) {
		$inserts[] = "('$tagid','$explainingid')";
	}
	if($inserts) $_SGLOBAL['db']->query("REPLACE INTO ".tname('tagexplaining')." (tagid,explainingid) VALUES ".implode(',', $inserts));
...
//ÊÓÆµ±êÇ©´¦Àí
function explaining_bbcode($message) {
	$message = preg_replace("/\[flash\=?(media|real)*\](.+?)\[\/flash\]/ie", "explaining_flash('\\2', '\\1')", $message);
	return $message;
}
//ÊÓÆµ
function explaining_flash($swf_url, $type='') {
	$width = '520';
	$height = '390';
	if ($type == 'media') {
