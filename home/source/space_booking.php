...
//预约页面
$query1=$_SGLOBAL['db']->query("SELECT * FROM ".tname('booking')." where bookingid='$id'");
$list1=$_SGLOBAL['db']->fetch_array($query1);

$query=$_SGLOBAL['db']->query("SELECT * FROM ".tname('bookingjoin')." where bookingid='$id' order by id DESC limit 5");
	$commentcount1=0;
	while($value=$_SGLOBAL['db']->fetch_array($query))
	{
		$list[]=$value;
	...
	if($_GET['view'] == 'click') {
		...
		if($_GET['view'] == 'all') {
			...
			if($_GET['orderby']) {
				...
				$theurl = "space.php?uid=$space[uid]&do=booking&view=all&orderby=$_GET[orderby]";
	...
	$_TPL['css'] = 'booking';
	include_once template("space_booking_list");
}
?> 