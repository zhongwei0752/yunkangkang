...
/*
	...
	$Id: function_dial.php 13245 2009-08-25 02:01:40Z liguode $
*/
...
//添加博客
function dial_post($POST, $olds=array()) {
	...
	if(empty($olds['classid']) || $POST['classid'] != $olds['classid']) {
		if(!empty($POST['classid']) && substr($POST['classid'], 0, 4) == 'new:') {
			...
			if(empty($classname)) {
				...
				$classid = getcount('classdial', array('classname'=>$classname, 'uid'=>$_SGLOBAL['supe_uid']), 'classid');
				if(empty($classid)) {
					$setarr = array(
						'classname' => $classname,
					...
					$classid = inserttable('classdial', $setarr, 1);
				}
			}
		} else {
	...
	$dialarr = array(
		...
		'secondprice'=>$POST['secondprice'],
		'thirdprice'=>$POST['thirdprice'],
		'fristrand'=>$POST['fristrand'],
		'secondrand'=>$POST['secondrand'],
		'thirdrand'=>$POST['thirdrand'],
		'frequency'=>$POST['frequency'],
		'frequencynum'=>$POST['frequencynum'],
		'classid' => $classid,
		'friend' => $POST['friend'],
	...
	if(!empty($POST['picids'])) {
		...
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			if(empty($titlepic) && $value['thumb']) {
				...
				$dialarr['picflag'] = $value['remote']?2:1;
			}
	...
	$dialarr['pic'] = $titlepic;
	
	//热度
	if(checkperm('managedial')) {
		$dialarr['hot'] = intval($POST['hot']);
	}
	
	if($olds['dialid']) {
		//更新
		$dialid = $olds['dialid'];
		updatetable('dial', $dialarr, array('dialid'=>$dialid));
		
		$fuids = array();
		
		$dialarr['uid'] = $olds['uid'];
		$dialarr['username'] = $olds['username'];
	} else {
		//参与热闹
		$dialarr['topicid'] = topic_check($POST['topicid'], 'dial');

		$dialarr['uid'] = $_SGLOBAL['supe_uid'];
		$dialarr['username'] = $_SGLOBAL['supe_username'];
		$dialarr['dateline'] = empty($POST['dateline'])?$_SGLOBAL['timestamp']:$POST['dateline'];
		$dialid = inserttable('dial', $dialarr, 1);
	}
	
	$dialarr['dialid'] = $dialid;
	$message1=str_replace("attachment","http://v5.home3d.cn/home/attachment",$message);
	$message1=str_replace("http://v5.home3d.cn/home/http://v5.home3d.cn/home/attachment","http://v5.home3d.cn/home/attachment/",$message1);	//附表	
	$fieldarr = array(
		'message' => $message,
		'message1' => $message1,
	...
	if($POST['tag'] != $oldtagstr) {
		if(!empty($olds['tag'])) {
			//先把以前的给清理掉
			$oldtags = array();
			$query = $_SGLOBAL['db']->query("SELECT tagid, dialid FROM ".tname('tagdial')." WHERE dialid='$dialid'");
			while ($value = $_SGLOBAL['db']->fetch_array($query)) {
				$oldtags[] = $value['tagid'];
			}
			if($oldtags) {
				$_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET dialnum=dialnum-1 WHERE tagid IN (".simplode($oldtags).")");
				$_SGLOBAL['db']->query("DELETE FROM ".tname('tagdial')." WHERE dialid='$dialid'");
			}
		}
		$tagarr = tag_batch($dialid, $POST['tag']);
		//更新附表中的tag
	...
	if($olds) {
		//更新
		updatetable('dialfield', $fieldarr, array('dialid'=>$dialid));
	} else {
		$fieldarr['dialid'] = $dialid;
		$fieldarr['uid'] = $dialarr['uid'];
		inserttable('dialfield', $fieldarr);
	}
	...
	if($isself) {
		if($olds) {
			...
			if(empty($space['dialnum'])) {
				$space['dialnum'] = getcount('dial', array('uid'=>$space['uid']));
				$dialnumsql = "dialnum=".$space['dialnum'];
			} else {
				$dialnumsql = 'dialnum=dialnum+1';
			}
			//积分
			$reward = getreward('publishdial', 0);
			$_SGLOBAL['db']->query("UPDATE ".tname('space')." SET {$dialnumsql}, lastpost='$_SGLOBAL[timestamp]', updatetime='$_SGLOBAL[timestamp]', credit=credit+$reward[credit], experience=experience+$reward[experience] WHERE uid='$_SGLOBAL[supe_uid]'");
	...
	include("./source/upload.class.php");
  	$image= new upload;
  	$image->upload_file($dialid,"dial");
	//产生feed
	if($POST['makefeed']) {
		include_once(S_ROOT.'./source/function_feed.php');
		feed_publish($dialid, 'dialid', $olds?0:1);
	}
	
	//热闹
	if(empty($olds) && $dialarr['topicid']) {
		topic_join($dialarr['topicid'], $_SGLOBAL['supe_uid'], $_SGLOBAL['supe_username']);
	}
...
//处理tag
function tag_batch($dialid, $tags) {
	global $_SGLOBAL;
	...
	if($updatetagids) $_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET dialnum=dialnum+1 WHERE tagid IN (".simplode($updatetagids).")");
	$tagids = array_keys($tagarr);
	$inserts = array();
	foreach ($tagids as $tagid) {
		$inserts[] = "('$tagid','$dialid')";
	}
	if($inserts) $_SGLOBAL['db']->query("REPLACE INTO ".tname('tagdial')." (tagid,dialid) VALUES ".implode(',', $inserts));
...
//视频标签处理
function dial_bbcode($message) {
	$message = preg_replace("/\[flash\=?(media|real)*\](.+?)\[\/flash\]/ie", "dial_flash('\\2', '\\1')", $message);
	return $message;
}
//视频
function dial_flash($swf_url, $type='') {
	$width = '520';
	$height = '390';
	if ($type == 'media') {
