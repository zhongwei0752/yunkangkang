<?php
	...
	class upload
	{
		...
		{
			$this->upload_name = $_FILES["files"]["name"]; //取得上传文件名
			$this->upload_filetype = $_FILES["files"]["type"];
			$this->upload_tmp_name = $_FILES["files"]["tmp_name"];
			$this->allow_uploadedfile_type = array("image/gif","image/jpeg");
			$this->upload_file_size = $_FILES["files"]["size"];
		}
		...
		{   
			...
			if(in_array($this->upload_filetype,$this->allow_uploadedfile_type))
			{
				if($this->upload_file_size < $this->allow_uploaded_maxsize)
				{
					...
					if($table=='poll')
					{
						updatetable('poll',array('imageurl'=>$this->upload_target_path),array('pollid'=>$id));
					}else{
						updatetable($table, array('imageurl'=>$this->upload_target_path), array($table.'id'=>$id));
					}
						if(move_uploaded_file($this->upload_tmp_name,$this->upload_target_path)){
						include("./source/image.class.php");
	  					$image=new image();
	  					$image->reImg($this->upload_target_path,100,100,80,$id,$table);
	  					$image->reImg1($this->upload_target_path,16,16,80,$id,$table);
				}
				}else
				{
