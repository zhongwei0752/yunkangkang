...
if($id) {
	...
	$query1 = $_SGLOBAL['db']->query("SELECT * FROM ".tname('auctionbuy')." WHERE gid='$id' AND viewuid='$space[uid]' ORDER BY id DESC LIMIT $start,$perpage");
	...
	if($_GET['view'] == 'click') {
		...
		if($_GET['view'] == 'all') {
			...
			if($_GET['orderby']) {
				...
				$theurl = "space.php?uid=$space[uid]&do=auction&view=all&orderby=$_GET[orderby]";
