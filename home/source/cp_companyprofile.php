...
/*
	...
	$Id: cp_companyprofile.php 13149 2009-08-13 03:11:26Z liguode $
*/
...
$theurl = "cp.php?ac=companyprofile&op=$_GET[op]";

if($_GET['op'] == 'base') {
	
	if(submitcheck('companyprofilesubmit') || submitcheck('nextsubmit')) {
		
		if(!@include_once(S_ROOT.'./data/data_companyprofilefield.php')) {
			include_once(S_ROOT.'./source/function_cache.php');
			companyprofilefield_cache();
		}
		$companyprofilefields = empty($_SGLOBAL['companyprofilefield'])?array():$_SGLOBAL['companyprofilefield'];
		...
		foreach ($companyprofilefields as $field => $value) {
		...
		//产生feed
		if(ckprivacy('companyprofile', 1)) {
			feed_add('companyprofile', cplang('feed_companyprofile_update_base'));
		}
	
		if(submitcheck('nextsubmit')) {
			$url = 'cp.php?ac=companyprofile&op=contact';
		} else {
			$url = 'cp.php?ac=companyprofile&op=base';
		}
	...
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('companyprofilefield')." ORDER BY displayorder");
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		...
		$companyprofilefields[$value['fieldid']] = $value;
	}
	...
	if($_GET['resend']) {
		...
		showmessage('do_success', "cp.php?ac=companyprofile&op=contact");
	}
	...
	if(submitcheck('companyprofilesubmit') || submitcheck('nextsubmit')) {
		...
		//产生feed
		if(ckprivacy('companyprofile', 1)) {
			feed_add('companyprofile', cplang('feed_companyprofile_update_contact'));
		}
		
		if(submitcheck('nextsubmit')) {
			$url = 'cp.php?ac=companyprofile&op=edu';
		} else {
			$url = 'cp.php?ac=companyprofile&op=contact';
		}
		showmessage('update_on_successful_individuals', $url);
	...
	if(submitcheck('companyprofilesubmit') || submitcheck('nextsubmit')) {
		...
		//产生feed
		if(ckprivacy('companyprofile', 1)) {
			feed_add('companyprofile', cplang('feed_companyprofile_update_edu'));
		}

		if(submitcheck('nextsubmit')) {
			$url = 'cp.php?ac=companyprofile&op=work';
		} else {
			$url = 'cp.php?ac=companyprofile&op=edu';
		}
		showmessage('update_on_successful_individuals', $url);
	...
	if(submitcheck('companyprofilesubmit') || submitcheck('nextsubmit')) {
		...
		//产生feed
		if(ckprivacy('companyprofile', 1)) {
			feed_add('companyprofile', cplang('feed_companyprofile_update_work'));
		}


		if(submitcheck('nextsubmit')) {
			$url = 'cp.php?ac=companyprofile&op=info';
		} else {
			$url = 'cp.php?ac=companyprofile&op=work';
		}
		showmessage('update_on_successful_individuals', $url);
	...
	if(submitcheck('companyprofilesubmit')) {
		...
		//产生feed
		if(ckprivacy('companyprofile', 1)) {
			feed_add('companyprofile', cplang('feed_companyprofile_update_info'));
		}
...
include template("cp_companyprofile");
