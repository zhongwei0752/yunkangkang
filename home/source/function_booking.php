...
/*
	...
	$Id: function_booking.php 13245 2009-08-25 02:01:40Z liguode $
*/
...
//Ìí¼Ó²©¿Í
function booking_post($POST, $olds=array()) {
	...
	if(empty($olds['classid']) || $POST['classid'] != $olds['classid']) {
		if(!empty($POST['classid']) && substr($POST['classid'], 0, 4) == 'new:') {
			...
			if(empty($classname)) {
				...
				$classid = getcount('classbooking', array('classname'=>$classname, 'uid'=>$_SGLOBAL['supe_uid']), 'classid');
				if(empty($classid)) {
					$setarr = array(
						'classname' => $classname,
					...
					$classid = inserttable('classbooking', $setarr, 1);
				}
			}
		} else {
	...
	if($classid && empty($classname)) {
		//ÊÇ·ñÊÇ×Ô¼ºµÄ
		$classname = getcount('classbooking', array('classid'=>$classid, 'uid'=>$_SGLOBAL['supe_uid']), 'classname');
	...
	if(!empty($POST['picids'])) {
		...
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			if(empty($titlepic) && $value['thumb']) {
				...
				$bookingarr['picflag'] = $value['remote']?2:1;
			}
	...
	if(empty($titlepic)) {
		...
		$bookingarr['picflag'] = 0;
	}
	$bookingarr['pic'] = $titlepic;
	
	//ÈÈ¶È
	if(checkperm('managebooking')) {
		$bookingarr['hot'] = intval($POST['hot']);
	}
	if($olds['bookingid']) {
		//¸üÐÂ
		$bookingid = $olds['bookingid'];
		updatetable('booking', $bookingarr, array('bookingid'=>$bookingid));
		
		$fuids = array();
		
		$bookingarr['uid'] = $olds['uid'];
		$bookingarr['username'] = $olds['username'];
	} else {
		//²ÎÓëÈÈÄÖ
		$bookingarr['topicid'] = topic_check($POST['topicid'], 'booking');

		$bookingarr['uid'] = $_SGLOBAL['supe_uid'];
		$bookingarr['username'] = $_SGLOBAL['supe_username'];
		$bookingarr['dateline'] = empty($POST['dateline'])?$_SGLOBAL['timestamp']:$POST['dateline'];
		$bookingid = inserttable('booking', $bookingarr, 1);
	}
	...
	if($POST['tag'] != $oldtagstr) {
		if(!empty($olds['tag'])) {
			//ÏÈ°ÑÒÔÇ°µÄ¸øÇåÀíµô
			$oldtags = array();
			$query = $_SGLOBAL['db']->query("SELECT tagid, bookingid FROM ".tname('tagbooking')." WHERE bookingid='$bookingid'");
			while ($value = $_SGLOBAL['db']->fetch_array($query)) {
				$oldtags[] = $value['tagid'];
			}
			if($oldtags) {
				$_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET bookingnum=bookingnum-1 WHERE tagid IN (".simplode($oldtags).")");
				$_SGLOBAL['db']->query("DELETE FROM ".tname('tagbooking')." WHERE bookingid='$bookingid'");
			}
		}
		$tagarr = tag_batch($bookingid, $POST['tag']);
		//¸üÐÂ¸½±íÖÐµÄtag
	...
	if($olds) {
		//¸üÐÂ
		updatetable('bookingfield', $fieldarr, array('bookingid'=>$bookingid));
	} else {
		$fieldarr['bookingid'] = $bookingid;
		$fieldarr['uid'] = $bookingarr['uid'];
		inserttable('bookingfield', $fieldarr);
	}
	...
	if($isself) {
		if($olds) {
			...
			if(empty($space['bookingnum'])) {
				$space['bookingnum'] = getcount('booking', array('uid'=>$space['uid']));
				$bookingnumsql = "bookingnum=".$space['bookingnum'];
			} else {
				$bookingnumsql = 'bookingnum=bookingnum+1';
			}
			//»ý·Ö
			$reward = getreward('publishbooking', 0);
			$_SGLOBAL['db']->query("UPDATE ".tname('space')." SET {$bookingnumsql}, lastpost='$_SGLOBAL[timestamp]', updatetime='$_SGLOBAL[timestamp]', credit=credit+$reward[credit], experience=experience+$reward[experience] WHERE uid='$_SGLOBAL[supe_uid]'");
	...
	include("./source/upload.class.php");
  	$image= new upload;
  	$image->upload_file($bookingid,"booking");
	//²úÉúfeed
	if($POST['makefeed']) {
		include_once(S_ROOT.'./source/function_feed.php');
		feed_publish($bookingid, 'bookingid', $olds?0:1);
	}
	
	//ÈÈÄÖ
	if(empty($olds) && $bookingarr['topicid']) {
		topic_join($bookingarr['topicid'], $_SGLOBAL['supe_uid'], $_SGLOBAL['supe_username']);
	}
...
//´¦Àítag
function tag_batch($bookingid, $tags) {
	global $_SGLOBAL;
	...
	if($updatetagids) $_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET bookingnum=bookingnum+1 WHERE tagid IN (".simplode($updatetagids).")");
	$tagids = array_keys($tagarr);
	$inserts = array();
	foreach ($tagids as $tagid) {
		$inserts[] = "('$tagid','$bookingid')";
	}
	if($inserts) $_SGLOBAL['db']->query("REPLACE INTO ".tname('tagbooking')." (tagid,bookingid) VALUES ".implode(',', $inserts));
...
//ÊÓÆµ±êÇ©´¦Àí
function booking_bbcode($message) {
	$message = preg_replace("/\[flash\=?(media|real)*\](.+?)\[\/flash\]/ie", "booking_flash('\\2', '\\1')", $message);
	return $message;
}
//ÊÓÆµ
function booking_flash($swf_url, $type='') {
	$width = '520';
	$height = '390';
	if ($type == 'media') {
