...
/*
	...
	$Id: function_code.php 13245 2009-08-25 02:01:40Z liguode $
*/
...
//Ìí¼Ó²©¿Í
function code_post($POST, $olds=array()) {
	...
	if(empty($olds['classid']) || $POST['classid'] != $olds['classid']) {
		if(!empty($POST['classid']) && substr($POST['classid'], 0, 4) == 'new:') {
			...
			if(empty($classname)) {
				...
				$classid = getcount('classcode', array('classname'=>$classname, 'uid'=>$_SGLOBAL['supe_uid']), 'classid');
				if(empty($classid)) {
					$setarr = array(
						'classname' => $classname,
					...
					$classid = inserttable('classcode', $setarr, 1);
				}
			}
		} else {
	...
	if($classid && empty($classname)) {
		//ÊÇ·ñÊÇ×Ô¼ºµÄ
		$classname = getcount('classcode', array('classid'=>$classid, 'uid'=>$_SGLOBAL['supe_uid']), 'classname');
	...
	if($POST['push1']){
		$codearr['push']=$POST['push1'];
	}
	if($POST['push']){
		$codearr['push']=$POST['push'];
	}
	//±êÌâÍ¼Æ¬
	$titlepic = '';
	...
	if(!empty($POST['picids'])) {
		...
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			if(empty($titlepic) && $value['thumb']) {
				...
				$codearr['picflag'] = $value['remote']?2:1;
			}
	...
	$codearr['pic'] = $titlepic;
	
	//ÈÈ¶È
	if(checkperm('managecode')) {
		$codearr['hot'] = intval($POST['hot']);
	}
	
	if($olds['codeid']) {
		//¸üÐÂ
		$codeid = $olds['codeid'];
		updatetable('code', $codearr, array('codeid'=>$codeid));
		
		$fuids = array();
		
		$codearr['uid'] = $olds['uid'];
		$codearr['username'] = $olds['username'];
	} else {
		//²ÎÓëÈÈÄÖ
		$codearr['topicid'] = topic_check($POST['topicid'], 'code');

		$codearr['uid'] = $_SGLOBAL['supe_uid'];
		$codearr['username'] = $_SGLOBAL['supe_username'];
		$codearr['dateline'] = empty($POST['dateline'])?$_SGLOBAL['timestamp']:$POST['dateline'];
		$codeid = inserttable('code', $codearr, 1);
	}
	
	$codearr['codeid'] = $codeid;
	...
	if($POST['tag'] != $oldtagstr) {
		if(!empty($olds['tag'])) {
			//ÏÈ°ÑÒÔÇ°µÄ¸øÇåÀíµô
			$oldtags = array();
			$query = $_SGLOBAL['db']->query("SELECT tagid, codeid FROM ".tname('tagcode')." WHERE codeid='$codeid'");
			while ($value = $_SGLOBAL['db']->fetch_array($query)) {
				$oldtags[] = $value['tagid'];
			}
			if($oldtags) {
				$_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET codenum=codenum-1 WHERE tagid IN (".simplode($oldtags).")");
				$_SGLOBAL['db']->query("DELETE FROM ".tname('tagcode')." WHERE codeid='$codeid'");
			}
		}
		$tagarr = tag_batch($codeid, $POST['tag']);
		//¸üÐÂ¸½±íÖÐµÄtag
	...
	if($olds) {
		//¸üÐÂ
		updatetable('codefield', $fieldarr, array('codeid'=>$codeid));
	} else {
		$fieldarr['codeid'] = $codeid;
		$fieldarr['uid'] = $codearr['uid'];
		inserttable('codefield', $fieldarr);
	}
	//¿Õ¼ä¸üÐÂ
	if($isself) {
		if($olds) {
			...
			if(empty($space['codenum'])) {
				$space['codenum'] = getcount('code', array('uid'=>$space['uid']));
				$codenumsql = "codenum=".$space['codenum'];
			} else {
				$codenumsql = 'codenum=codenum+1';
			}
			//»ý·Ö
			$reward = getreward('publishcode', 0);
			$_SGLOBAL['db']->query("UPDATE ".tname('space')." SET {$codenumsql}, lastpost='$_SGLOBAL[timestamp]', updatetime='$_SGLOBAL[timestamp]', credit=credit+$reward[credit], experience=experience+$reward[experience] WHERE uid='$_SGLOBAL[supe_uid]'");
	...
	include("./source/upload.class.php");
  	$image= new upload;
  	$image->upload_file($codeid,"code");
	//²úÉúfeed
	if($POST['makefeed']) {
		include_once(S_ROOT.'./source/function_feed.php');
		feed_publish($codeid, 'codeid', $olds?0:1);
	}
	
	//ÈÈÄÖ
	if(empty($olds) && $codearr['topicid']) {
		topic_join($codearr['topicid'], $_SGLOBAL['supe_uid'], $_SGLOBAL['supe_username']);
	}
...
//´¦Àítag
function tag_batch($codeid, $tags) {
	global $_SGLOBAL;
	...
	if($updatetagids) $_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET codenum=codenum+1 WHERE tagid IN (".simplode($updatetagids).")");
	$tagids = array_keys($tagarr);
	$inserts = array();
	foreach ($tagids as $tagid) {
		$inserts[] = "('$tagid','$codeid')";
	}
	if($inserts) $_SGLOBAL['db']->query("REPLACE INTO ".tname('tagcode')." (tagid,codeid) VALUES ".implode(',', $inserts));
...
//ÊÓÆµ±êÇ©´¦Àí
function code_bbcode($message) {
	$message = preg_replace("/\[flash\=?(media|real)*\](.+?)\[\/flash\]/ie", "code_flash('\\2', '\\1')", $message);
	return $message;
}
//ÊÓÆµ
function code_flash($swf_url, $type='') {
	$width = '520';
	$height = '390';
	if ($type == 'media') {
