...
//预约页面
$query1=$_SGLOBAL['db']->query("SELECT * FROM ".tname('explaining')." where explainingid='$id'");
$list1=$_SGLOBAL['db']->fetch_array($query1);

$query=$_SGLOBAL['db']->query("SELECT * FROM ".tname('explainingjoin')." where explainingid='$id' order by id DESC limit 5");
	$commentcount1=0;
	while($value=$_SGLOBAL['db']->fetch_array($query))
	{
		$list[]=$value;
	...
	$_TPL['css'] = 'explaining';
	include_once template("space_explain");*/
} else {
	//·ÖÒ³
	$perpage = 5;
	$perpage = mob_perpage($perpage);
	...
	if($_GET['view'] == 'click') {
		...
		if($_GET['view'] == 'all') {
			...
			if($_GET['orderby']) {
				...
				$theurl = "space.php?uid=$space[uid]&do=explaining&view=all&orderby=$_GET[orderby]";
	...
	$_TPL['css'] = 'explaining';
	include_once template("space_explaining_list");
}
?> 