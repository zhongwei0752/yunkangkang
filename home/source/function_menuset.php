...
/*
	...
	$Id: function_menuset.php 13245 2009-08-25 02:01:40Z liguode $
*/
...
//添加博客
function menuset_post($POST, $olds=array()) {
	...
	if(empty($olds['classid']) || $POST['classid'] != $olds['classid']) {
		if(!empty($POST['classid']) && substr($POST['classid'], 0, 4) == 'new:') {
			...
			if(empty($classname)) {
				...
				$classid = getcount('classmenuset', array('classname'=>$classname, 'uid'=>$_SGLOBAL['supe_uid']), 'classid');
				if(empty($classid)) {
					$setarr = array(
						'classname' => $classname,
					...
					$classid = inserttable('classmenuset', $setarr, 1);
				}
			}
		} else {
	...
	if($classid && empty($classname)) {
		//是否是自己的
		$classname = getcount('classmenuset', array('classid'=>$classid, 'uid'=>$_SGLOBAL['supe_uid']), 'classname');
	...
	if(!empty($POST['picids'])) {
		...
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			if(empty($titlepic) && $value['thumb']) {
				...
				$menusetarr['picflag'] = $value['remote']?2:1;
			}
	...
	if(empty($titlepic)) {
		...
		$menusetarr['picflag'] = 0;
	}
	$menusetarr['pic'] = $titlepic;
	
	//热度
	if(checkperm('managemenuset')) {
		$menusetarr['hot'] = intval($POST['hot']);
	}
	
	if($olds['menusetid']) {
		//更新
		$menusetid = $olds['menusetid'];
		updatetable('menuset', $menusetarr, array('menusetid'=>$menusetid));
		
		$fuids = array();
		
		$menusetarr['uid'] = $olds['uid'];
		$menusetarr['username'] = $olds['username'];
	} else {
		//参与热闹
		$menusetarr['topicid'] = topic_check($POST['topicid'], 'menuset');

		$menusetarr['uid'] = $_SGLOBAL['supe_uid'];
		$menusetarr['username'] = $_SGLOBAL['supe_username'];
		$menusetarr['dateline'] = empty($POST['dateline'])?$_SGLOBAL['timestamp']:$POST['dateline'];
		$menusetid = inserttable('menuset', $menusetarr, 1);
	}
	
	$menusetarr['menusetid'] = $menusetid;
	...
	if($POST['tag'] != $oldtagstr) {
		if(!empty($olds['tag'])) {
			//先把以前的给清理掉
			$oldtags = array();
			$query = $_SGLOBAL['db']->query("SELECT tagid, menusetid FROM ".tname('tagmenuset')." WHERE menusetid='$menusetid'");
			while ($value = $_SGLOBAL['db']->fetch_array($query)) {
				$oldtags[] = $value['tagid'];
			}
			if($oldtags) {
				$_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET menusetnum=menusetnum-1 WHERE tagid IN (".simplode($oldtags).")");
				$_SGLOBAL['db']->query("DELETE FROM ".tname('tagmenuset')." WHERE menusetid='$menusetid'");
			}
		}
		$tagarr = tag_batch($menusetid, $POST['tag']);
		//更新附表中的tag
	...
	if($olds) {
		//更新
		updatetable('menusetfield', $fieldarr, array('menusetid'=>$menusetid));
	} else {
		$fieldarr['menusetid'] = $menusetid;
		$fieldarr['uid'] = $menusetarr['uid'];
		inserttable('menusetfield', $fieldarr);
	}
	...
	if($isself) {
		if($olds) {
			...
			if(empty($space['menusetnum'])) {
				$space['menusetnum'] = getcount('menuset', array('uid'=>$space['uid']));
				$menusetnumsql = "menusetnum=".$space['menusetnum'];
			} else {
				$menusetnumsql = 'menusetnum=menusetnum+1';
			}
			//积分
			$reward = getreward('publishmenuset', 0);
			$_SGLOBAL['db']->query("UPDATE ".tname('space')." SET {$menusetnumsql}, lastpost='$_SGLOBAL[timestamp]', updatetime='$_SGLOBAL[timestamp]', credit=credit+$reward[credit], experience=experience+$reward[experience] WHERE uid='$_SGLOBAL[supe_uid]'");
	...
	include("./source/upload.class.php");
  	$image= new upload;
  	$image->upload_file($menusetid,"menuset");

	//产生feed
	if($POST['makefeed']) {
		include_once(S_ROOT.'./source/function_feed.php');
		feed_publish($menusetid, 'menusetid', $olds?0:1);
	}
	
	//热闹
	if(empty($olds) && $menusetarr['topicid']) {
		topic_join($menusetarr['topicid'], $_SGLOBAL['supe_uid'], $_SGLOBAL['supe_username']);
	}
...
//处理tag
function tag_batch($menusetid, $tags) {
	global $_SGLOBAL;
	...
	if($updatetagids) $_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET menusetnum=menusetnum+1 WHERE tagid IN (".simplode($updatetagids).")");
	$tagids = array_keys($tagarr);
	$inserts = array();
	foreach ($tagids as $tagid) {
		$inserts[] = "('$tagid','$menusetid')";
	}
	if($inserts) $_SGLOBAL['db']->query("REPLACE INTO ".tname('tagmenuset')." (tagid,menusetid) VALUES ".implode(',', $inserts));
...
//视频标签处理
function menuset_bbcode($message) {
	$message = preg_replace("/\[flash\=?(media|real)*\](.+?)\[\/flash\]/ie", "menuset_flash('\\2', '\\1')", $message);
	return $message;
}
//视频
function menuset_flash($swf_url, $type='') {
	$width = '520';
	$height = '390';
	if ($type == 'media') {
