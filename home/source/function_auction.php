...
/*
	...
	$Id: function_auction.php 13245 2009-08-25 02:01:40Z liguode $
*/
...
//Ìí¼Ó²©¿Í
function auction_post($POST, $olds=array()) {
	...
	if(empty($olds['classid']) || $POST['classid'] != $olds['classid']) {
		if(!empty($POST['classid']) && substr($POST['classid'], 0, 4) == 'new:') {
			...
			if(empty($classname)) {
				...
				$classid = getcount('classauction', array('classname'=>$classname, 'uid'=>$_SGLOBAL['supe_uid']), 'classid');
				if(empty($classid)) {
					$setarr = array(
						'classname' => $classname,
					...
					$classid = inserttable('classauction', $setarr, 1);
				}
			}
		} else {
	...
	if($classid && empty($classname)) {
		//ÊÇ·ñÊÇ×Ô¼ºµÄ
		$classname = getcount('classauction', array('classid'=>$classid, 'uid'=>$_SGLOBAL['supe_uid']), 'classname');
	...
	$auctionarr = array(
		...
		"starttime" => sstrtotime($POST['starttime']),
		"endtime" => sstrtotime($POST['endtime']),
		'fristprice'=> $POST['fristprice'],
		'auctionpassword'=>$POST['auctionpassword'],
		'plusprice'=>$POST['plusprice'],
		'classid' => $classid,
		'friend' => $POST['friend'],
	...
	if(!empty($POST['picids'])) {
		...
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			if(empty($titlepic) && $value['thumb']) {
				...
				$auctionarr['picflag'] = $value['remote']?2:1;
			}
	...
	if(empty($titlepic)) {
		...
		$auctionarr['picflag'] = 0;
	}
	$auctionarr['pic'] = $titlepic;
	
	//ÈÈ¶È
	if(checkperm('manageauction')) {
		$auctionarr['hot'] = intval($POST['hot']);
	}
	if($olds['auctionid']) {
		//¸üÐÂ
		$auctionid = $olds['auctionid'];
		updatetable('auction', $auctionarr, array('auctionid'=>$auctionid));
		
		$fuids = array();
		
		$auctionarr['uid'] = $olds['uid'];
		$auctionarr['username'] = $olds['username'];
	} else {
		//²ÎÓëÈÈÄÖ
		$auctionarr['topicid'] = topic_check($POST['topicid'], 'auction');

		$auctionarr['uid'] = $_SGLOBAL['supe_uid'];
		$auctionarr['username'] = $_SGLOBAL['supe_username'];
		$auctionarr['dateline'] = empty($POST['dateline'])?$_SGLOBAL['timestamp']:$POST['dateline'];
		$auctionid = inserttable('auction', $auctionarr, 1);
	}
	
	$auctionarr['auctionid'] = $auctionid;
	...
	if($POST['tag'] != $oldtagstr) {
		if(!empty($olds['tag'])) {
			//ÏÈ°ÑÒÔÇ°µÄ¸øÇåÀíµô
			$oldtags = array();
			$query = $_SGLOBAL['db']->query("SELECT tagid, auctionid FROM ".tname('tagauction')." WHERE auctionid='$auctionid'");
			while ($value = $_SGLOBAL['db']->fetch_array($query)) {
				$oldtags[] = $value['tagid'];
			}
			if($oldtags) {
				$_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET auctionnum=auctionnum-1 WHERE tagid IN (".simplode($oldtags).")");
				$_SGLOBAL['db']->query("DELETE FROM ".tname('tagauction')." WHERE auctionid='$auctionid'");
			}
		}
		$tagarr = tag_batch($auctionid, $POST['tag']);
		//¸üÐÂ¸½±íÖÐµÄtag
	...
	if($olds) {
		//¸üÐÂ
		updatetable('auctionfield', $fieldarr, array('auctionid'=>$auctionid));
	} else {
		$fieldarr['auctionid'] = $auctionid;
		$fieldarr['uid'] = $auctionarr['uid'];
		inserttable('auctionfield', $fieldarr);
	}
	...
	if($isself) {
		if($olds) {
			...
			if(empty($space['auctionnum'])) {
				$space['auctionnum'] = getcount('auction', array('uid'=>$space['uid']));
				$auctionnumsql = "auctionnum=".$space['auctionnum'];
			} else {
				$auctionnumsql = 'auctionnum=auctionnum+1';
			}
			//»ý·Ö
			$reward = getreward('publishauction', 0);
			$_SGLOBAL['db']->query("UPDATE ".tname('space')." SET {$auctionnumsql}, lastpost='$_SGLOBAL[timestamp]', updatetime='$_SGLOBAL[timestamp]', credit=credit+$reward[credit], experience=experience+$reward[experience] WHERE uid='$_SGLOBAL[supe_uid]'");
	...
	include("./source/upload.class.php");
  	$image= new upload;
  	$image->upload_file($auctionid,"auction");
	//²úÉúfeed
	if($POST['makefeed']) {
		include_once(S_ROOT.'./source/function_feed.php');
		feed_publish($auctionid, 'auctionid', $olds?0:1);
	}
	
	//ÈÈÄÖ
	if(empty($olds) && $auctionarr['topicid']) {
		topic_join($auctionarr['topicid'], $_SGLOBAL['supe_uid'], $_SGLOBAL['supe_username']);
	}
...
//´¦Àítag
function tag_batch($auctionid, $tags) {
	global $_SGLOBAL;
	...
	if($updatetagids) $_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET auctionnum=auctionnum+1 WHERE tagid IN (".simplode($updatetagids).")");
	$tagids = array_keys($tagarr);
	$inserts = array();
	foreach ($tagids as $tagid) {
		$inserts[] = "('$tagid','$auctionid')";
	}
	if($inserts) $_SGLOBAL['db']->query("REPLACE INTO ".tname('tagauction')." (tagid,auctionid) VALUES ".implode(',', $inserts));
...
//ÊÓÆµ±êÇ©´¦Àí
function auction_bbcode($message) {
	$message = preg_replace("/\[flash\=?(media|real)*\](.+?)\[\/flash\]/ie", "auction_flash('\\2', '\\1')", $message);
	return $message;
}
//ÊÓÆµ
function auction_flash($swf_url, $type='') {
	$width = '520';
	$height = '390';
	if ($type == 'media') {
