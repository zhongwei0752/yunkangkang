...
/*
	...
	$Id: function_introduce.php 13245 2009-08-25 02:01:40Z liguode $
*/
...
//Ìí¼Ó²©¿Í
function introduce_post($POST, $olds=array()) {
	...
	if(empty($olds['classid']) || $POST['classid'] != $olds['classid']) {
		if(!empty($POST['classid']) && substr($POST['classid'], 0, 4) == 'new:') {
			...
			if(empty($classname)) {
				...
				$classid = getcount('classintroduce', array('classname'=>$classname, 'uid'=>$_SGLOBAL['supe_uid']), 'classid');
				if(empty($classid)) {
					$setarr = array(
						'classname' => $classname,
					...
					$classid = inserttable('classintroduce', $setarr, 1);
				}
			}
		} else {
	...
	if(empty($titlepic)) {
		...
		$introducearr['picflag'] = 0;
	}
	$introducearr['pic'] = $titlepic;
	...
	if($olds['introduceid']) {
		...
		$introducearr['username'] = $_SGLOBAL['supe_username'];
		$introducearr['dateline'] = empty($POST['dateline'])?$_SGLOBAL['timestamp']:$POST['dateline'];
		$introduceid = inserttable('introduce', $introducearr, 1);
	}
	...
	if($POST['tag'] != $oldtagstr) {
		if(!empty($olds['tag'])) {
			//ÏÈ°ÑÒÔÇ°µÄ¸øÇåÀíµô
			$oldtags = array();
			$query = $_SGLOBAL['db']->query("SELECT tagid, introduceid FROM ".tname('tagintroduce')." WHERE introduceid='$introduceid'");
			while ($value = $_SGLOBAL['db']->fetch_array($query)) {
				$oldtags[] = $value['tagid'];
			}
			if($oldtags) {
				$_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET introducenum=introducenum-1 WHERE tagid IN (".simplode($oldtags).")");
				$_SGLOBAL['db']->query("DELETE FROM ".tname('tagintroduce')." WHERE introduceid='$introduceid'");
			}
		}
		$tagarr = tag_batch($introduceid, $POST['tag']);
		//¸üÐÂ¸½±íÖÐµÄtag
	...
	if($isself) {
		if($olds) {
			...
			if(empty($space['introducenum'])) {
				$space['introducenum'] = getcount('introduce', array('uid'=>$space['uid']));
				$introducenumsql = "introducenum=".$space['introducenum'];
			} else {
				$introducenumsql = 'introducenum=introducenum+1';
			}
			//»ý·Ö
			$reward = getreward('publishintroduce', 0);
			$_SGLOBAL['db']->query("UPDATE ".tname('space')." SET {$introducenumsql}, lastpost='$_SGLOBAL[timestamp]', updatetime='$_SGLOBAL[timestamp]', credit=credit+$reward[credit], experience=experience+$reward[experience] WHERE uid='$_SGLOBAL[supe_uid]'");
			
			//Í³¼Æ
			updatestat('introduce');
		}
	}
	...
	if($POST['makefeed']) {
		...
		feed_publish($introduceid, 'introduceid', $olds?0:1);
	}
	
	//ÈÈÄÖ
	if(empty($olds) && $introducearr['topicid']) {
		topic_join($introducearr['topicid'], $_SGLOBAL['supe_uid'], $_SGLOBAL['supe_username']);
	}
...
//´¦Àítag
function tag_batch($introduceid, $tags) {
	global $_SGLOBAL;
	...
	if($updatetagids) $_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET introducenum=introducenum+1 WHERE tagid IN (".simplode($updatetagids).")");
	$tagids = array_keys($tagarr);
	$inserts = array();
	foreach ($tagids as $tagid) {
		$inserts[] = "('$tagid','$introduceid')";
	}
	if($inserts) $_SGLOBAL['db']->query("REPLACE INTO ".tname('tagintroduce')." (tagid,introduceid) VALUES ".implode(',', $inserts));
...
//ÊÓÆµ±êÇ©´¦Àí
function introduce_bbcode($message) {
	$message = preg_replace("/\[flash\=?(media|real)*\](.+?)\[\/flash\]/ie", "introduce_flash('\\2', '\\1')", $message);
	return $message;
}
//ÊÓÆµ
function introduce_flash($swf_url, $type='') {
	$width = '520';
	$height = '390';
	if ($type == 'media') {
