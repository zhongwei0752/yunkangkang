...
//图片上传
	if($_POST['menusetimage']){
	$menusetid=$_POST['menusetid'];
	include("./source/upload6.class.php");
  	$image= new upload;
  	$image->upload_file($menusetid,"menusetimage");
  	showmessage("上传成功","space.php?do=menuset&view=me&change=3");
...
if($_GET['view'] != 'me') {
	...
	if($showmessage){
		if($_GET['status']=='taocan'){
			if($_GET['id']=='putong'){
			updatetable("space",array('groupid'=>'12'),array('uid'=>$_SGLOBAL['supe_uid']));	
			}elseif($_GET['id']=='gaoji'){
			updatetable("space",array('groupid'=>'13'),array('uid'=>$_SGLOBAL['supe_uid']));	
			}else{
			updatetable("space",array('groupid'=>'11'),array('uid'=>$_SGLOBAL['supe_uid']));
			}
