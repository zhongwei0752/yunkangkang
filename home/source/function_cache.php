...
//更新用户栏目缓存
function messagefield_cache() {
	global $_SGLOBAL;

	$_SGLOBAL['messagefield'] = array();
	$query = $_SGLOBAL['db']->query('SELECT fieldid, title, formtype, maxsize, required, invisible, allowsearch, choice FROM '.tname('messagefield')." ORDER BY displayorder");
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		$_SGLOBAL['messagefield'][$value['fieldid']] = $value;
	}
	cache_write('messagefield', "_SGLOBAL['messagefield']", $_SGLOBAL['messagefield']);
}
