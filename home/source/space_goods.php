...
if($status=='sort'){
	$i='1';
	$query1 = $_SGLOBAL['db']->query("SELECT sort FROM ".tname('goods')." WHERE uid='$_SGLOBAL[supe_uid]' order by dateline DESC");
	$value1 = $_SGLOBAL['db']->fetch_array($query1);
	if(empty($value1)){

		$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('goods')." WHERE uid='$_SGLOBAL[supe_uid]' order by dateline DESC");
	while($value = $_SGLOBAL['db']->fetch_array($query)){
		updatetable("goods",array("sort"=>$i),array('goodsid'=>$value['goodsid']));
		$i++;
	}
	}
	
}elseif($status=='up'){//上移一位
	$goodsid=$_GET['goodsid'];
	$upsortid=$sortid-1;
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('goods')." WHERE uid='$_SGLOBAL[supe_uid]' and sort='$upsortid'");
	$value = $_SGLOBAL['db']->fetch_array($query);
	updatetable("goods",array("sort"=>$upsortid),array("uid"=>$_SGLOBAL['supe_uid'],"goodsid"=>$goodsid));
	updatetable("goods",array("sort"=>$sortid),array("uid"=>$_SGLOBAL['supe_uid'],"goodsid"=>$value['goodsid']));
	showmessage("修改成功","space.php?do=goods&view=me&status=sort");
}elseif($status=='down'){//下移一位
	$goodsid=$_GET['goodsid'];
	$upsortid=$sortid+1;
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('goods')." WHERE uid='$_SGLOBAL[supe_uid]' and sort='$upsortid'");
	$value = $_SGLOBAL['db']->fetch_array($query);
	...
	updatetable("goods",array("sort"=>$sortid),array("uid"=>$_SGLOBAL['supe_uid'],"goodsid"=>$value['goodsid']));
	showmessage("修改成功","space.php?do=goods&view=me&status=sort");
}elseif($status=='top'){//置顶
	$goodsid=$_GET['goodsid'];
	$_SGLOBAL['db']->query("UPDATE ".tname('goods')." SET sort=sort+1 WHERE uid='$_SGLOBAL[supe_uid]' and sort<'$sortid'");
	//$_SGLOBAL['db']->query("UPDATE ".tname('goods')." SET sort=sort-1 WHERE uid='$_SGLOBAL[supe_uid]' and sort>'$sortid'");
	updatetable("goods",array("sort"=>'1'),array("uid"=>$_SGLOBAL['supe_uid'],"goodsid"=>"$goodsid"));
	showmessage("置顶成功","space.php?do=goods&view=me&status=sort");
}
//各模块小logo
...
if($id) {
	...
	ckstart($start, $perpage);
	$list = array();
	$count = $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('goodscod')."  WHERE viewuid='$_SGLOBAL[supe_uid]' and status='0'"),0);
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('goodscod')."
		WHERE viewuid='$_SGLOBAL[supe_uid]' and status='0' Order By dateline DESC
		LIMIT $start,$perpage");
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
	...
	$theurl="space.php?do=goods&cod=2";		
	$perpage = 10;
	$perpage = mob_perpage($perpage);
	...
	ckstart($start, $perpage);
	$list = array();
	$count = $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('goodscod')."  WHERE viewuid='$_SGLOBAL[supe_uid]' and status='1'"),0);
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('goodscod')."
		WHERE viewuid='$_SGLOBAL[supe_uid]' and status='1' Order By dateline1 DESC
		LIMIT $start,$perpage");
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		$query1 = $_SGLOBAL['db']->query("SELECT bf.message, bf.target_ids, bf.magiccolor, b.* FROM ".tname('goods')." b 
				LEFT JOIN ".tname('goodsfield')." bf ON bf.goodsid=b.goodsid
