...
var prebackground;
var precolor;
document.writeln("<div id='Calendar' style='position:absolute; z-index:10000; visibility: hidden;'></div>");
    ...
    if(getElement.tagName == "TD"){
        if(getElement.innerHTML != ""){
			if(type == 1){
				...
				sDate = $('Year').value + "-" + $('Month').value + "-" + getElement.innerHTML;
...
function GetDate2(idname){
	sDate = $('Year').value + "-" + $('Month').value + "-" + seltd.innerHTML + " " + $('Hour').value + ":" + $('Minute').value;
	$(idname).value=sDate;
    HiddenCalendar();
}
...
function ShowCalendar(idname,type){
    ...
    table="<table border='0' cellspacing='0' style='width:250px;background:#fff url(image/menubg.gif) center top repeat-x;border:1px solid #a9d5e9;'>";
    table+="<tr>";
    table+="<td style='padding:5px 5px 4px 8px;border-bottom:1px solid;background: url(image/h.png) repeat-x;text-align:left;padding:0.5em; line-height:2.4em;'>";
    table+="<span style='float:right;cursor:pointer;' title='关闭' onClick='javascript:HiddenCalendar()'><img src=\"image/dclose.gif\" /></span>";
    ...
    table+="</table></td></tr>";
	if(type == 1){
		table+="<tr><td align=\"center\" style=\"padding:1px;\"><input style=\"font:12px Arial;padding:1px 3px 0 3px;vertical-align:middle;margin-bottom:1px;border: 1px solid; border-color: #c0c0c0 #ededed #ededed #c0c0c0;padding:2px 0px 2px 1px;font-size:1.0em;vertical-align:middle;color:#000;\" type=\"text\" name=\"Hour\" id=\"Hour\" size=\"1\" value=\""+today.hour+"\"> 点 <input style=\"font:12px Arial;padding:1px 3px 0 3px;vertical-align:middle;margin-bottom:1px;border: 1px solid; border-color: #c0c0c0 #ededed #ededed #c0c0c0;padding:2px 0px 2px 1px;font-size:1.0em;vertical-align:middle;color:#000;\" type=\"text\" name=\"minute\" id=\"Minute\" size=\"1\" value=\""+today.minute+"\"> 分 <input style=\"border-color: #4c9542 #668f01 #668f01 #4c9542;margin-right:1em;color:#fff;background:#89bf01;border-width:1px;cursor:pointer;padding:.1em 1em;*padding:0 1em;font-size:9pt; line-height:130%; overflow:visible;\" type=\"button\" name=\"submit\" value=\"确定\" onClick='GetDate2(\"" + idname + "\")'></td></tr>";
	}
	table+="</table>";
    Cal.innerHTML=table;
    ShowDays();
