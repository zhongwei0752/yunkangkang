...
//²úÉú¶¯Ì¬
function feed_publish($id, $idtype, $add=0,$uid,$typeid) {
	global $_SGLOBAL;
	...
	switch ($idtype) {
		...
		case 'viewid':
			...
			if($value = $_SGLOBAL['db']->fetch_array($query)) {
				...
				$query2 = $_SGLOBAL['db']->query("SELECT * FROM ".tname('space')."
				WHERE uid='$uid'");
				$newid=$id."id";
				$space1 = $_SGLOBAL['db']->fetch_array($query2);
				$query2 = $_SGLOBAL['db']->query("SELECT * FROM ".tname($id)."
				WHERE $newid = '$typeid'");
				$value1 = $_SGLOBAL['db']->fetch_array($query2);
 			$setarr = array(
			    'icon' =>'profile',
			    'uid' => $uid,
			    'fatherid'=>$add,
				...
				'idtype' => $id,
				'friend' => 0,
				'title_template' => "$space[name] 查看了$space1[name]的$value[subject]<$value1[subject]>",
			   );		  
	...
	if($setarr['icon']) {
		...
		//Êý¾Ý´¦Àí
		$setarr['title_data'] = serialize($setarr['title_data']);//Êý×é×ª»¯
		if($idtype != 'sid') {
			$setarr['body_data'] = serialize($setarr['body_data']);//Êý×é×ª»¯
		}
