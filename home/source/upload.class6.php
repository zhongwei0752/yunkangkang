<?php
	...
	class upload6
	{
		...
		{
			$this->upload_name = $_FILES["refiles"]["name"]; //取得上传文件名
			$this->upload_filetype = $_FILES["refiles"]["type"];
			$this->upload_tmp_name = $_FILES["refiles"]["tmp_name"];
			$this->allow_uploadedfile_type = array("image/gif","image/jpeg");
			$this->upload_file_size = $_FILES["refiles"]["size"];
		}
		...
		{   
			...
			if(in_array($this->upload_filetype,$this->allow_uploadedfile_type))
			{
				...
				{
					$this->upload_target_dir="./upload/$table/redebate/";
					...
					updatetable($table,array('reimageurl'=>$this->upload_target_path),array($table.'id'=>$id));
