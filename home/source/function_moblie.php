...
/*
	...
	$Id: function_moblie.php 13245 2009-08-25 02:01:40Z liguode $
*/
...
//Ìí¼Ó²©¿Í
function moblie_post($POST, $olds=array()) {
	...
	if(empty($olds['classid']) || $POST['classid'] != $olds['classid']) {
		if(!empty($POST['classid']) && substr($POST['classid'], 0, 4) == 'new:') {
			...
			if(empty($classname)) {
				...
				$classid = getcount('classmoblie', array('classname'=>$classname, 'uid'=>$_SGLOBAL['supe_uid']), 'classid');
				if(empty($classid)) {
					$setarr = array(
						'classname' => $classname,
					...
					$classid = inserttable('classmoblie', $setarr, 1);
				}
			}
		} else {
	...
	if($classid && empty($classname)) {
		//ÊÇ·ñÊÇ×Ô¼ºµÄ
		$classname = getcount('classmoblie', array('classid'=>$classid, 'uid'=>$_SGLOBAL['supe_uid']), 'classname');
	...
	if(!empty($POST['picids'])) {
		...
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			if(empty($titlepic) && $value['thumb']) {
				...
				$mobliearr['picflag'] = $value['remote']?2:1;
			}
	...
	$mobliearr['pic'] = $titlepic;
	
	//ÈÈ¶È
	if(checkperm('managemoblie')) {
		$mobliearr['hot'] = intval($POST['hot']);
	}

	if($olds['moblieid']) {
		//¸üÐÂ
		$moblieid = $olds['moblieid'];
		updatetable('moblie', $mobliearr, array('moblieid'=>$moblieid));
		
		$fuids = array();
		
		$mobliearr['uid'] = $olds['uid'];
		$mobliearr['username'] = $olds['username'];
	} else {
		//²ÎÓëÈÈÄÖ
		$mobliearr['topicid'] = topic_check($POST['topicid'], 'moblie');

		$mobliearr['uid'] = $_SGLOBAL['supe_uid'];
		$mobliearr['username'] = $_SGLOBAL['supe_username'];
		$mobliearr['dateline'] = empty($POST['dateline'])?$_SGLOBAL['timestamp']:$POST['dateline'];
		$moblieid = inserttable('moblie', $mobliearr, 1);
	}
	
	$mobliearr['moblieid'] = $moblieid;
	...
	if($POST['tag'] != $oldtagstr) {
		if(!empty($olds['tag'])) {
			//ÏÈ°ÑÒÔÇ°µÄ¸øÇåÀíµô
			$oldtags = array();
			$query = $_SGLOBAL['db']->query("SELECT tagid, moblieid FROM ".tname('tagmoblie')." WHERE moblieid='$moblieid'");
			while ($value = $_SGLOBAL['db']->fetch_array($query)) {
				$oldtags[] = $value['tagid'];
			}
			if($oldtags) {
				$_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET moblienum=moblienum-1 WHERE tagid IN (".simplode($oldtags).")");
				$_SGLOBAL['db']->query("DELETE FROM ".tname('tagmoblie')." WHERE moblieid='$moblieid'");
			}
		}
		$tagarr = tag_batch($moblieid, $POST['tag']);
		//¸üÐÂ¸½±íÖÐµÄtag
		$fieldarr['tag'] = empty($tagarr)?'':addslashes(serialize($tagarr));
	}
if($olds) {
		//¸üÐÂ
		updatetable('mobliefield', $fieldarr, array('moblieid'=>$moblieid));
	} else {
		$fieldarr['moblieid'] = $moblieid;
		$fieldarr['uid'] = $mobliearr['uid'];
		inserttable('mobliefield', $fieldarr);
	}
	...
	if($isself) {
		if($olds) {
			...
			if(empty($space['moblienum'])) {
				$space['moblienum'] = getcount('moblie', array('uid'=>$space['uid']));
				$moblienumsql = "moblienum=".$space['moblienum'];
			} else {
				$moblienumsql = 'moblienum=moblienum+1';
			}
			//»ý·Ö
			$reward = getreward('publishmoblie', 0);
			$_SGLOBAL['db']->query("UPDATE ".tname('space')." SET {$moblienumsql}, lastpost='$_SGLOBAL[timestamp]', updatetime='$_SGLOBAL[timestamp]', credit=credit+$reward[credit], experience=experience+$reward[experience] WHERE uid='$_SGLOBAL[supe_uid]'");
	...
	include("./source/upload.class.php");
  	$image= new upload;
  	$image->upload_file($moblieid,"moblie");

	//²úÉúfeed
	if($POST['makefeed']) {
		include_once(S_ROOT.'./source/function_feed.php');
		feed_publish($moblieid, 'moblieid', $olds?0:1);
	}
	
	//ÈÈÄÖ
	if(empty($olds) && $mobliearr['topicid']) {
		topic_join($mobliearr['topicid'], $_SGLOBAL['supe_uid'], $_SGLOBAL['supe_username']);
	}
...
//´¦Àítag
function tag_batch($moblieid, $tags) {
	global $_SGLOBAL;
	...
	if($updatetagids) $_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET moblienum=moblienum+1 WHERE tagid IN (".simplode($updatetagids).")");
	$tagids = array_keys($tagarr);
	$inserts = array();
	foreach ($tagids as $tagid) {
		$inserts[] = "('$tagid','$moblieid')";
	}
	if($inserts) $_SGLOBAL['db']->query("REPLACE INTO ".tname('tagmoblie')." (tagid,moblieid) VALUES ".implode(',', $inserts));
...
//ÊÓÆµ±êÇ©´¦Àí
function moblie_bbcode($message) {
	$message = preg_replace("/\[flash\=?(media|real)*\](.+?)\[\/flash\]/ie", "moblie_flash('\\2', '\\1')", $message);
	return $message;
}
//ÊÓÆµ
function moblie_flash($swf_url, $type='') {
	$width = '520';
	$height = '390';
	if ($type == 'media') {
