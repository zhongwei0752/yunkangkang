...
$op = empty($_REQUEST['op']) ? "edit" : $_REQUEST['op'];
...
if(submitcheck('eventsubmit')) {
	...
	// ����Ϣ
	$arr1 = array(
		"title" => getstr($_REQUEST['title'], 80, 1, 1, 1),
		"classid" => intval($_REQUEST['classid']),
		"province" => getstr($_REQUEST['province'], 20, 1, 1),
		"city" => getstr($_REQUEST['city'], 20, 1, 1),
		"location" => getstr($_REQUEST['location'], 80, 1, 1, 1),
		"starttime" => sstrtotime($_REQUEST['starttime']),
		"endtime" => sstrtotime($_REQUEST['endtime']),
		"deadline" => sstrtotime($_REQUEST['deadline']),
		"public" => intval($_REQUEST['public'])
	);
	// ��չ��Ϣ
	$arr2 = array(
		"detail" => getstr($_REQUEST['detail'], '', 1, 1, 1, 0, 1),
		"limitnum" => intval($_REQUEST['limitnum']),
		"verify" => intval($_REQUEST['verify']),
		"allowpost" => intval($_REQUEST['allowpost']),
		"allowpic" => intval($_REQUEST['allowpic']),
		"allowfellow" => intval($_REQUEST['allowfellow']),
		"allowinvite" => intval($_REQUEST['allowinvite']),
		"template" => getstr($_REQUEST['template'], 255, 1, 1, 1)
	);
	
	//�������
	if(empty($arr1['title'])){
		capi_showmessage_by_data('event_title_empty');
	} elseif(empty($arr1['classid'])){
		capi_showmessage_by_data('event_classid_empty');
	} elseif(empty($arr1['city'])) {
		capi_showmessage_by_data('event_city_empty');
	} elseif(empty($arr2['detail'])) {
		capi_showmessage_by_data('event_detail_empty');
	} elseif($arr1['endtime']-$arr1['starttime']>60 * 24 * 3600) {
		capi_showmessage_by_data('event_bad_time_range');		
	} elseif($arr1['endtime']<$arr1['starttime']) {
		capi_showmessage_by_data('event_bad_endtime');
	} elseif($arr1['deadline']>$arr1['endtime']) {
		capi_showmessage_by_data('event_bad_deadline');
	} elseif(!$eventid && $arr1['starttime']<$_SGLOBAL['timestamp']) {
		capi_showmessage_by_data('event_bad_starttime');
	}
	...
	//����Ⱥ��
	if($_REQUEST['tagid'] && (!$eventid || $event['uid']==$_SGLOBAL['supe_uid']) && $_REQUEST['tagid'] != $event['tagid']) {
		$_REQUEST['tagid'] = intval($_REQUEST['tagid']);
		$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname("tagspace")." WHERE tagid='$_REQUEST[tagid]' AND uid='$_SGLOBAL[supe_uid]' LIMIT 1");
	...
	if($eventid){// �޸����л
		if($allowmanage){
			...
			// ���?��
			if($_REQUEST['sharepic'] && !empty($pic['picid'])){
				$arr = array(
					"eventid"=>$eventid,
					"picid"=>$pic['picid'],
			...
			capi_showmessage_by_data('do_success', 'space.php?do=event&id='.$eventid, 0);			
		} else {
			capi_showmessage_by_data('no_privilege_edit_event');
		}
		...
		$_REQUEST['topicid'] = topic_check($_REQUEST['topicid'], 'event');
		$arr1['topicid'] = $_REQUEST['topicid'];
		...
		if (! $eventid){
			capi_showmessage_by_data("event_create_failed"); // �����ʧ�ܣ����������������
		}
		// ���Ϣ
		...
		if($_REQUEST['topicid']) {
			...
			$url = 'space.php?do=topic&topicid='.$_REQUEST['topicid'].'&view=event';
		} else {
			$url = 'space.php?do=event&id='.$eventid;
		}
		
		capi_showmessage_by_data('do_success', $url, 0); // �鿴�
	}
}
...
if($op == 'invite') {
	...
	if((!$event['allowinvite'] && $_SGLOBAL['supe_userevent']['status'] < 3) || ($_SGLOBAL['supe_userevent']['status'] < 2)){
		capi_showmessage_by_data("no_privilege_do_eventinvite");
	}
	...
	if(submitcheck('invitesubmit')){
		...
		$inserts = array();
		$touids = array();
		for($i=0, $L=sizeof($_REQUEST['ids']); $i<$L; $i++){
			$arr['touid'] = intval($_REQUEST['ids'][$i]);
			$arr['tousername'] = getstr($_REQUEST['names'][$i], 15, 1, 1);
		...
		$_REQUEST['page'] = empty($_REQUEST['page'])?0:intval($_REQUEST['page']);
		capi_showmessage_by_data("do_success", "cp.php?ac=event&op=invite&id=$eventid&group=$_REQUEST[group]&page=$_REQUEST[page]", 2);
	}
	...
	$multi = multi($count, $perpage, $page, "cp.php?ac=event&op=invite&id=$eventid&group=$_REQUEST[group]&key=$_REQUEST[key]");

} elseif($op == 'members') {
	// ��Ա����

	if($_SGLOBAL['supe_userevent']['status'] < 3){
		capi_showmessage_by_data('no_privilege_manage_event_members');//��û��Ȩ�޹�����Ա
	}
	...
	if(submitcheck("memberssubmit")){
		...
		if($_REQUEST['ids'] && verify_eventmembers($_REQUEST['ids'], $_REQUEST['status'])){
			capi_showmessage_by_data("do_success", "cp.php?ac=event&op=members&id=$eventid&status=$_REQUEST[status]", 2);
		} else {
			capi_showmessage_by_data("choose_right_eventmember", "cp.php?ac=event&op=members&id=$eventid&status=$_REQUEST[status]", 5);
		}
	}
	...
	if($_REQUEST['key']) {
		$_REQUEST['key'] = stripsearchkey($_REQUEST['key']);
		$wheresql = " AND username LIKE '%$_REQUEST[key]%' ";
	} else {
		$_REQUEST['status'] = intval($_REQUEST['status']);
		$wheresql = " AND status='$_REQUEST[status]'";		
	}
	...
	if($_REQUEST['key']){
		$_REQUEST['status'] = $list[0]['status'];
	}

	$multi = smulti($start, $perpage, $count, "cp.php?ac=event&op=members&id=$eventid&status=$_REQUEST[status]&key=$_REQUEST[key]");
	...
	if(submitcheck("membersubmit")){
		$_REQUEST['status'] = intval($_REQUEST['status']);
		if($_REQUEST['uid'] && verify_eventmembers(array($_REQUEST['uid']), $_REQUEST['status'])){
			$refer = empty($_REQUEST['refer']) ? "space.php?do=event&id=$eventid&view=member&status=$_REQUEST[status]" : $_REQUEST['refer'];
			capi_showmessage_by_data("do_success", $refer , 0);	
		} else {
			capi_showmessage_by_data("choose_right_eventmember");
		}
	}
	
	$_REQUEST['uid'] = intval($_REQUEST['uid']);
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname("userevent")." WHERE uid='$_REQUEST[uid]' AND eventid='$eventid'");
	...
	if(!$allowmanage){
		capi_showmessage_by_data("no_privilege_manage_event_comment");
	}

	capi_showmessage_by_data("redirect", "admincp.php?ac=comment&idtype=eventid&id=$eventid", 0);

} elseif($op == 'pic') {// ���Ƭ

	if(!$allowmanage){
		capi_showmessage_by_data("no_privilege_manage_event_pic");
	}

	if(submitcheck("deletepicsubmit")){
		if(! empty($_REQUEST['ids'])) {
			$query = $_SGLOBAL['db']->query("DELETE FROM " . tname("eventpic") . " WHERE eventid='$eventid' AND picid IN (".simplode($_REQUEST['ids']).")");
			...
			capi_showmessage_by_data("do_success", "cp.php?ac=event&op=pic&id=$eventid", 0);
		} else {
			capi_showmessage_by_data("choose_event_pic");
		}
	}
	...
	if(!$allowmanage){
		capi_showmessage_by_data("no_privilege_manage_event_thread");
	}
	if(!$event['tagid']) {
		capi_showmessage_by_data('event_has_not_mtag');//�û�й���Ⱥ��
	}
	
	if(submitcheck('delthreadsubmit')) {
		if(!empty($_REQUEST['ids'])) {
			$_SGLOBAL['db']->query("DELETE FROM ".tname("thread")." WHERE eventid='$eventid' AND tid IN (".simplode($_REQUEST['ids']).")");
			...
			capi_showmessage_by_data('do_success',"cp.php?ac=event&id=$eventid&op=thread",0);
		} else {
			capi_showmessage_by_data('choose_event_thread');
		}
	}
	...
	if(empty($_SGLOBAL['supe_userevent'])){
		...
		if($_SGLOBAL['timestamp'] > $event['endtime']){	
			capi_showmessage_by_data('event_is_over');// ��Ѿ�����
		}
		
		if($_SGLOBAL['timestamp'] > $event['deadline']){
			capi_showmessage_by_data("event_meet_deadline"); // ��Ѿ���ֹ����
		}
		
		if($event['limitnum']>0 && $event['membernum']>=$event['limitnum']){
			capi_showmessage_by_data('event_already_full');//���������
		}
		...
		if($event['public'] < 2){
			...
			$value = $_SGLOBAL['db']->fetch_array($query);
			if(empty($value)){				
				capi_showmessage_by_data("event_join_limit"); // �˻ֻ��ͨ��������ܼ���
			}
		}
	}
	...
	if(submitcheck("joinsubmit")){
		...
		if(!empty($_SGLOBAL['supe_userevent']) && $_SGLOBAL['supe_userevent']['status'] == 0){
			...
			if(isset($_REQUEST['fellow'])){
				$arr['fellow'] = intval($_REQUEST['fellow']);// �޸�Я������
			}
			if($_REQUEST['template']){// ������Ϣ
				$arr['template'] = getstr($_REQUEST['template'], 255, 1, 1, 1);
			}
			if($arr){
				updatetable("userevent", $arr, array("eventid"=>$eventid, "uid"=>$_SGLOBAL['supe_uid']));
			}
			capi_showmessage_by_data("do_success", "space.php?do=event&id=$eventid", 2);
		}
		...
		if(!empty($_SGLOBAL['supe_userevent']) && $_SGLOBAL['supe_userevent']['status'] > 1){
			...
			if(isset($_REQUEST['fellow'])){// �޸�Я������
				$_REQUEST['fellow'] = intval($_REQUEST['fellow']);
				$arr['fellow'] = $_REQUEST['fellow'];// �޸Ĳμ�����
				$num = $_REQUEST['fellow'] - $_SGLOBAL['supe_userevent']['fellow'];
				// �������
			...
			if($_REQUEST['template']){// ������Ϣ
				$arr['template'] = $_REQUEST['template'];
			}
			if($arr){
				updatetable("userevent", $arr, array("eventid"=>$eventid, "uid"=>$_SGLOBAL['supe_uid']));
			}
		...
		if($_REQUEST['fellow']){
			$arr['fellow'] = intval($_REQUEST['fellow']);
			$num += $arr['fellow'];
		}
		if($_REQUEST['template']){// ������Ϣ
			$arr['template'] = getstr($_REQUEST['template'], 255, 1, 1, 1);
		...
		if ($event['limitnum'] > 0 && $num + $event['membernum'] > $event['limitnum']){
			capi_showmessage_by_data("event_will_full");
		}
		...
		capi_showmessage_by_data("do_success", "space.php?do=event&id=$eventid", 0); // �����ɹ�
	}
	...
	if(submitcheck("quitsubmit")){
		...
		if(! empty($userevent) && $event['uid'] != $uid){
			...
			capi_showmessage_by_data("do_success", $tourl, 0);
		} else {
			capi_showmessage_by_data("cannot_quit_event", $tourl, 2); // �㲻���˳����ԭ�����㻹û�м����������������ķ����ˡ�
		}
	}
	...
	if(!empty($_SGLOBAL['supe_userevent'])){
		$_REQUEST['popupmenu_box'] = true;//�����ر�
		if($_SGLOBAL['supe_userevent']['status']<=1) {
			capi_showmessage_by_data("event_has_followed");//���Ѿ���ע�˴˻
		} else {
			capi_showmessage_by_data("event_has_joint");//���Ѿ������˴˻
		}
	}
	...
	if($_REQUEST['r']) {// �ܾ�
		$tourl = "cp.php?ac=event&op=eventinvite" . (isset($_REQUEST['page']) ? "&page=" . intval($_REQUEST['page']) : "");	
		if($eventid) {// �����˻id
			$_SGLOBAL['db']->query("DELETE FROM ". tname("eventinvite") . " WHERE eventid = '$eventid' AND touid = '$_SGLOBAL[supe_uid]'");
			$_SGLOBAL['db']->query("UPDATE ".tname('space')." SET eventinvitenum=eventinvitenum-1 WHERE uid = '$_SGLOBAL[supe_uid]' AND eventinvitenum>0");
		} else {// ����
	...
	if(!$eventinvite) {
		capi_showmessage_by_data('eventinvite_does_not_exist');//��û�иû�Ļ����
	}
	...
	capi_showmessage_by_data(cplang('event_accept_success', array("space.php?do=event&id=$event[eventid]")));
	...
	if(submitcheck("deletesubmit")){
		...
		capi_showmessage_by_data("do_success", "space.php?do=event", 2);
	}	
	...
	if($event['grade'] < 1 || $event['endtime'] > $_SGLOBAL['timestamp']) {
		capi_showmessage_by_data('event_can_not_be_closed');
	}
	...
	if($event['grade'] != -2 || $event['endtime'] > $_SGLOBAL['timestamp']) {
		capi_showmessage_by_data('event_can_not_be_opened');
	}
	...
	$match = array();
	if(!$_REQUEST['month'] && preg_match("/^(\d{4}-\d{1,2})/", $_REQUEST['date'], $match)) {
		$_REQUEST['month'] = $match[1];
	}
	if(preg_match("/^(\d{4})-(\d{1,2})$/", $_REQUEST['month'], $match)){
		$year = intval($match[1]);
	...
	//����
	$url = $_REQUEST['url'] ? preg_replace("/date=[\d\-]+/", '', $_REQUEST['url']) : "space.php?do=event";
	...
	if(submitcheck('hotsubmit')) {
		...
		updatetable('event', array('hot'=>$_REQUEST['hot']), array('eventid'=>$eventid));
		
		if($_REQUEST['hot']>0) {
			include_once(S_ROOT.'./source/function_feed.php');
			feed_publish($eventid, 'eventid');
		} else {
			updatetable('feed', array('hot'=>$_REQUEST['hot']), array('id'=>$eventid, 'idtype'=>'eventid'));
		}
		capi_showmessage_by_data('do_success', "space.php?uid=$event[uid]&do=event&id=$eventid", 0);
	}
	...
	if($eventid) {
		...
		if(! checkperm("allowevent")){
		   capi_showmessage_by_data('no_privilege_add_event');
		}
	...
	if($_REQUEST['tagid'] && !$event['tagid']) {
		$event['tagid'] = intval($_REQUEST['tagid']);		
	}
...
function verify_eventmembers($uids, $status){
	...
	if($_SGLOBAL['supe_userevent']['status'] < 3){
		capi_showmessage_by_data('no_privilege_manage_event_members');
	}
	...
	if($status < -1 || $status > 3){
		capi_showmessage_by_data("bad_userevent_status"); // ��ѡ����ȷ�Ļ��Ա״̬
	}
	if($event['verify'] == 0 && $status == 0){
		capi_showmessage_by_data("event_not_set_verify");
	}
	if($status == 3 && $_SGLOBAL['supe_uid'] != $event['uid']){
		capi_showmessage_by_data("only_creator_can_set_admin"); // ֻ�д����߿��������Ա
	}
