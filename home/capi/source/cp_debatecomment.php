...
if(submitcheck('commentsubmit')) {
	...
	$comment = array();
	if($cid) {
		$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('debate_comment')." WHERE cid='$cid' AND debateid='$id' AND debatetype='$_REQUEST[debatetype]'");
	...
	//更新辩论
	 ...
	 $debatetype = empty($_REQUEST['debatetype'])?0:intval($_REQUEST['debatetype']);
	 }
	...
	capi_showmessage_by_data($msg, $_REQUEST['refer'], 0, $magvalues);
}

$cid = empty($_REQUEST['cid'])?0:intval($_REQUEST['cid']);

//编辑
if($_REQUEST['op'] == 'obedit' || $_REQUEST['op'] == 'reedit') {
...
}elseif($_REQUEST['op'] == 'obvoted' || $_REQUEST['op'] == 'revoted') {
       $query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('debate_comment')." WHERE cid='$cid' ");
	if(!$comment = $_SGLOBAL['db']->fetch_array($query)) {
		capi_showmessage_by_data('no_privilege');
	}
...
} elseif($_REQUEST['op'] == 'obreply' || $_REQUEST['op'] == 'rereply') {
