...
if ($_REQUEST["op"]=="check")
{
	...
	if(!$task = $_SGLOBAL['db']->fetch_array($query)) {
		capi_showmessage_by_data('task_unavailable');
	} else {
	...
	if(empty($task['done'])) {
		include_once(S_ROOT.'./capi/source/task/'.$task['filename']);
	}else{
		capi_showmessage_by_data('do_success', 0, array("done"=>$task['done']));
	}
	exit;
}elseif($_REQUEST["op"]=="finish"){
	...
	if($task['done']) {
			...
			capi_showmessage_by_data('rest_success', 0, array('credit'=>$task['credit']));
	}

	capi_showmessage_by_data('rest_fail');
}
...
if($taskid) {
	...
	if($_REQUEST['view'] == 'member') {
		...
		$theurl = "cp.php?ac=task&taskid=$taskid&view=$_REQUEST[view]";
		...
		//重新执行任务
		if($task['done'] && $task['ignore'] && $_REQUEST['op']=='redo') {
		...
		if(empty($task['done'])) {
			...
			if($_REQUEST['op'] == 'ignore') {
				//放弃任务
				$setarr['isignore'] = 1;
				inserttable('usertask', $setarr, 0, true);
				capi_showmessage_by_data('do_success',  0, $task);
			}
	...
	$list=array();
	foreach ($taskspacelist as $key=>$value)
	{
		$value['note'] = capi_fhtml($value['note']);
		$list[] = $value;
	}
...
capi_showmessage_by_data('do_success', 0, array("task"=>$list, "count"=>count($list)));
