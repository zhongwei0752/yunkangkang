...
if($id) {
	...
	if($_REQUEST['uid']=='1'){
		...
		WHERE viewuid='$_REQUEST[uid]' and gid='$_REQUEST[id]' and status='0' and moneystatus='1' order by dateline DESC
		LIMIT $start,$perpage");
	}else{
		$query1 = $_SGLOBAL['db']->query("SELECT * FROM ".tname('goodscod')."
	...
	while ($value1 = $_SGLOBAL['db']->fetch_array($query1)) {
		...
		$query3 = $_SGLOBAL['db']->query("SELECT c.* FROM ".tname('goodscod')." g LEFT JOIN ".tname('code')." c  ON g.gid=c.push WHERE g.gid='$value1[gid]'");
	...
	if($_REQUEST['view'] == 'click') {
		...
		if($_REQUEST['view'] == 'all') {
			...
			if($_REQUEST['orderby']) {
				...
				$theurl = "space.php?uid=$space[uid]&do=goods&view=all&orderby=$_REQUEST[orderby]";
		...
		if($count) {
			if(!empty($_REQUEST['status'])&&$_REQUEST['status']!='tanchuan'){
				$query = $_SGLOBAL['db']->query("SELECT bf.message, bf.target_ids, bf.magiccolor, b.* FROM ".tname('goods')." b $f_index
				LEFT JOIN ".tname('goodsfield')." bf ON bf.goodsid=b.goodsid
				WHERE b.uid='$_REQUEST[uid]' and b.fenlei='$_REQUEST[status]'
	...
	//ÊµÃû
	realname_get();
	capi_showmessage_by_data("rest_success", 0, array('goods'=>$list, 'count'=>$count));
	$_TPL['css'] = 'goods';
