...
function debate_post($POST, $olds=array()) {
	...
	if($olds['did']) {
		...
		updatetable('debate', $debatearr, array('did'=>$did));
		...
		$did = inserttable('debate', $debatearr, 1);
	}
	...
	if(empty($olds)) {
		...
		$fs['images'] = $fs['image_links'] = array();
			if($debatearr['pic']) {
				...
				$fs['image_links'] = array("space.php?uid=$_SGLOBAL[supe_uid]&do=debate&id=$did");
			}
			$fs['title_template'] =cplang('feed_debate');
			$fs['title_data'] = array(
				'subject' => "<a href=\"space.php?uid=$_SGLOBAL[supe_uid]&do=debate&id=$did\">$blogarr[subject]</a>"
			);
			...
			$fs['body_data'] = array(
				'subject' => "<a href=\"space.php?uid=$_SGLOBAL[supe_uid]&do=debate&id=$did\">$debatearr[subject]</a>",
