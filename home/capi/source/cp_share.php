...
if($_REQUEST['op'] == 'delete') {
	if(capi_submitcheck('deletesubmit')) {
		...
		capi_showmessage_by_data('do_success', $_REQUEST['type']=='view'?'space.php?do=share':$_REQUEST['refer'], 0);
	}
	...
	if(capi_submitcheck('hotsubmit')) {
		...
		updatetable('share', array('hot'=>$_REQUEST['hot']), array('sid'=>$sid));
		if($_REQUEST['hot']>0) {
			include_once(S_ROOT.'./source/function_feed.php');
			feed_publish($sid, 'sid');
		} else {
			updatetable('feed', array('hot'=>$_REQUEST['hot']), array('id'=>$sid, 'idtype'=>'sid'));
		}
	...
	switch ($type) {
		case 'space':
			...
			if($id == $space['uid']) {
				capi_showmessage_by_data('share_space_not_self');
			}
		...
		case 'blog':
			...
			if(!$blog = $_SGLOBAL['db']->fetch_array($query)) {
				capi_showmessage_by_data('blog_does_not_exist');
			}
			if($blog['uid'] == $space['uid']) {
				capi_showmessage_by_data('share_not_self');
			}
			if($blog['friend']) {
				capi_showmessage_by_data('logs_can_not_share');
			}
			//黑名单
			...
			break;
		case 'quiz':
			$query = $_SGLOBAL['db']->query("SELECT b.*,bf.message,bf.hotuser FROM ".tname('quiz')." b
				LEFT JOIN ".tname('quizfield')." bf ON bf.quizid=b.quizid
				WHERE b.quizid='$id'");
			if(!$quiz = $_SGLOBAL['db']->fetch_array($query)) {
				capi_showmessage_by_data('quiz_does_not_exist');
			}
			if($quiz['uid'] == $space['uid']) {
				capi_showmessage_by_data('share_not_self');
			}
			if($quiz['friend']) {
				capi_showmessage_by_data('logs_can_not_share');
			...
			//黑名单
			if(isblacklist($quiz['uid'])) {
				capi_showmessage_by_data('is_blacklist');
			}

			//实名
			realname_set($quiz['uid'], $quiz['username']);
			realname_get();

			$arr['title_template'] = cplang('share_quiz');
			$arr['body_template'] = '<b>{subject}</b><br>{username}<br>{message}';
			$arr['body_data'] = array(
				'subject' => "<a href=\"space.php?uid=$quiz[uid]&do=quiz&id=$quiz[quizid]\">$quiz[subject]</a>",
				'username' => "<a href=\"space.php?uid=$quiz[uid]\">".$_SN[$quiz['uid']]."</a>",
				'message' => getstr($quiz['message'], 150, 0, 1, 0, 0, -1)
			);
			if($quiz['pic']) {
				$arr['image'] = pic_cover_get($quiz['pic'], $quiz['picflag']);
				$arr['image_link'] = "space.php?uid=$quiz[uid]&do=quiz&id=$quiz[quizid]";
			}
			//通知
			$note_uid = $quiz['uid'];
			$note_message = cplang('note_share_quiz', array("space.php?uid=$quiz[uid]&do=quiz&id=$quiz[quizid]", $quiz['subject']));
		...
		case 'album':
			...
			if(!$album = $_SGLOBAL['db']->fetch_array($query)) {
				capi_showmessage_by_data('album_does_not_exist');
			}
			if($album['uid'] == $space['uid']) {
				capi_showmessage_by_data('share_not_self');
			}
			if($album['friend']) {
				capi_showmessage_by_data('album_can_not_share');
			}
			//黑名单
		...
		case 'pic':
			...
			if(!$pic = $_SGLOBAL['db']->fetch_array($query)) {
				capi_showmessage_by_data('image_does_not_exist');
			}
			if($pic['uid'] == $space['uid']) {
				capi_showmessage_by_data('share_not_self');
			}
			if($pic['friend']) {
				capi_showmessage_by_data('image_can_not_share');
			}
			//黑名单
		...
		case 'thread':
			...
			if(!$thread = $_SGLOBAL['db']->fetch_array($query)) {
				capi_showmessage_by_data('topics_does_not_exist');
			}
		...
		case 'mtag':
			...
			if(!$mtag = $_SGLOBAL['db']->fetch_array($query)) {
				capi_showmessage_by_data('designated_election_it_does_not_exist');
			}
		...
		case 'tag':
			...
			if(!$tag = $_SGLOBAL['db']->fetch_array($query)) {
				capi_showmessage_by_data('tag_does_not_exist');
			}
		...
		case 'poll':
			...
			if(!$poll = $_SGLOBAL['db']->fetch_array($query)) {
				capi_showmessage_by_data('poll_does_not_exist');
			}
	...
	//添加分享
	if(capi_submitcheck('sharesubmit')) {

		$_REQUEST['topicid'] = topic_check($_REQUEST['topicid'], 'share');
		
		//验证码
		if($type == 'link' && checkperm('seccode') && !ckseccode($_REQUEST['seccode'])) {
			capi_showmessage_by_data('incorrect_code');
		}

		if(empty($_REQUEST['refer'])) $_REQUEST['refer'] = "space.php?do=share&view=me";
		...
		if($type == 'link') {
			...
			if(empty($link)) {
				capi_showmessage_by_data('url_incorrect_format');
			}
		...
		if($note_uid && $note_uid != $_SGLOBAL['supe_uid']) {
			...
			notification_add_push($note_uid,  $note_message, $_SGLOBAL['supe_uid']);
		}
		...
		if($_REQUEST['topicid']) {
			...
			$url = 'space.php?do=topic&topicid='.$_REQUEST['topicid'].'&view=share';
		} else {
			$url = $_REQUEST['refer'];
		}
