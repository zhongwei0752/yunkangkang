...
$clickid = empty($_REQUEST['clickid'])?0:intval($_REQUEST['clickid']);
$idtype = empty($_REQUEST['idtype'])?'':trim($_REQUEST['idtype']);
...
if(empty($click)) {
	capi_showmessage_by_data('click_error');
}
...
switch ($idtype) {
	...
	case 'quizid':
		$idtype = 'quizid';
		$sql = "SELECT b.*, bf.hotuser FROM ".tname('quiz')." b
			LEFT JOIN ".tname('quizfield')." bf ON bf.quizid=b.quizid
			WHERE b.quizid='$id'";
		$tablename = tname('quiz');
		break;
	default:
		$idtype = 'blogid';
...
if(!$item = $_SGLOBAL['db']->fetch_array($query)) {
	capi_showmessage_by_data('click_item_error');
}
...
if($_REQUEST['op'] == 'add') {
	...
	if(!checkperm('allowclick') /*||$_REQUEST['hash'] != $hash*/ ) {
		capi_showmessage_by_data('no_privilege');
	}
	
	if($item['uid'] == $_SGLOBAL['supe_uid']) {
		capi_showmessage_by_data('click_no_self');
	}
	...
	if($value = $_SGLOBAL['db']->fetch_array($query)) {
		capi_showmessage_by_data('click_have');
	}
	...
	switch ($idtype) {
		case 'blogid':
			...
			break;
		case 'quizid':
			$fs['title_template'] = cplang('feed_click_quiz');
			$fs['title_data'] = array(
				'touser' => "<a href=\"space.php?uid=$item[uid]\">{$_SN[$item['uid']]}</a>",
				'subject' => "<a href=\"space.php?uid=$item[uid]&do=quiz&id=$item[quizid]\">$item[subject]</a>",
				'click' => $click['name']
			);
			$note_type = 'clickquiz';
			$q_note = cplang('note_click_quiz', array("space.php?uid=$item[uid]&do=quiz&id=$item[quizid]", $item['subject']));
			break;
		case 'tid':
	...
	notification_add_push($item['uid'],$q_note,$_SGLOBAL['supe_uid']);
	
	capi_showmessage_by_data('click_success', 0);
	
} elseif ($_REQUEST['op'] == 'show') {
