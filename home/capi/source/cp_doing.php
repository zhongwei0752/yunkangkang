...
if(empty($_REQUEST['refer'])) $_REQUEST['refer'] = "space.php?do=doing&view=me";

if(capi_submitcheck('addsubmit')) {

	$add_doing = 1;
	if(empty($_REQUEST['spacenote'])) {
...
} elseif (capi_submitcheck('commentsubmit')) {
	...
	if(empty($updo)) {
		capi_showmessage_by_data('docomment_error');
	} else {
		//黑名单
	...
	if($updo['uid'] != $_SGLOBAL['supe_uid']) {
		...
		notification_add_push($updo['uid'],  $note, $_SGLOBAL['supe_uid']);
