...
$acs = array(
	array('index','config', 'privacy', 'ip','image','weixin', 'spam', 'hotuser', 'defaultuser', 'usergroup', 'credit', 'magic', 'magiclog', 'profield', 'ad', 'userapp'),
	...
	array('introduce','product','development','industry','cases','branch','job','talk'));
...
if(empty($_GET['ac']) || (!in_array($_GET['ac'], $acs[0]) && !in_array($_GET['ac'], $acs[1]) && !in_array($_GET['ac'], $acs[2]) && !in_array($_GET['ac'], $acs[3])&& !in_array($_GET['ac'], $acs[4]))) {
	$ac = 'index';
} else {
	$ac = $_GET['ac'];
}
