...
/*
	...
	$Id: admincp_talk.php 12568 2009-07-08 07:38:01Z zhengqingpeng $
*/
...
//权限
if(!$allowmanage = checkperm('managetalk')) {
...
if(submitcheck('batchsubmit')) {
	...
	if(!empty($_POST['ids']) && deletetalks($_POST['ids'])) {
...
$mpurl = 'admincp.php?ac=talk';

//处理搜索
$intkeys = array('uid', 'friend', 'doid');
...
//排序
$orders = getorders(array('dateline', 'viewnum', 'replynum', 'hot'), 'doid', 'b.');
$ordersql = $orders['sql'];
...
if($perpage > 100) {
	...
	if($wheresql2) {
		$csql = "SELECT COUNT(*) FROM ".tname('talk')." b WHERE $wheresql  AND $wheresql2";
	} else {
		$csql = "SELECT COUNT(*) FROM ".tname('talk')." b WHERE $wheresql";
	}
...
if($wheresql2) {
	$qsql = "SELECT $selectsql FROM ".tname('talk')." b WHERE $wheresql  AND $wheresql2 ORDER BY b.doid DESC LIMIT $start,$perpage";
} else {
	$qsql = "SELECT $selectsql FROM ".tname('talk')." b WHERE $wheresql $ordersql LIMIT $start,$perpage";
}
if($count) {
