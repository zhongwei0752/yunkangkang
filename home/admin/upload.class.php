<?php
	...
	class upload
	{
		...
		function __construct()
		{
			...
			$this->allow_uploadedfile_type = array("image/gif","image/jpeg","image/png");
			$this->upload_file_size = $_FILES["file1"]["size"];
		}

		function upload_file($table,$image,$url)
		{   
			...
			if(in_array($this->upload_filetype,$this->allow_uploadedfile_type))
			{
				if($this->upload_file_size < $this->allow_uploaded_maxsize)
				{
					...
					if($image=="bigimage"){
					$this->upload_final_name = $table."_"."big.jpg";
					}
					if($image=="logoimage"){
						//showmessage($table.strtotime("now").$this->upload_name);
					$this->upload_final_name = $table.strtotime("now").".jpg";	
					}
					...
					if($image=="bigimage"){
  					updatetable($table, array('imageurl'=>$this->upload_target_path,'style' =>$image), array('id'=>'1'));
  				}
  				if($image=="logoimage"){
  					inserttable($table, array('imageurl'=>$this->upload_target_path,'style' =>$image,'url'=>$url));
  				}
