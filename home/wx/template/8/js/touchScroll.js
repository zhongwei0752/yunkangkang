...
TouchScroll.prototype={
	...
	resize:function(){
		...
		this.element=this.$C('div',{'class':'touchscrollelement','style':';overflow:hidden;width:'+this.scrollWidth+'px;padding:'
            +parseInt(barHeight/2)+'px '+parseInt(barWidth/2)+'px;position:absolute;top:-'+this.container.scrollTop+'px;left:-'+this.container.scrollLeft+'px;margin-left:61%;'}); //新加，让菜单位置在右边
		...
		this.horizontalScrollBar=this.$C('div',{'class':'touchscrollbar horizontal'/*,'style':scrollBarStyle*/});    //取消菜单上下滚动条的样式
		this.verticalScrollBar=this.$C('div',{'class':'touchscrollbar vertical'/*,'style':scrollBarStyle*/});        // 取消菜单上下滚动条的样式
