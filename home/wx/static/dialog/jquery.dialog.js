...
/**
 * 输入框
 * @param   {String, HTMLElement}   窗口标题
 * @param   {String, HTMLElement}   消息内容
 * @param   {Function}              确定按钮回调函数。函数第一个参数接收用户录入的数据
 * @param   {String}                输入框默认文本
 */
$.prompt = $.dialog.prompt = function (title, content, ok, defaultValue) {
    title        = title || '';
    ...
    return $.dialog({
        ...
        content: [
            ...
            '<div><input type="text" class="d-input-text" value="'+defaultValue+'" /></div>'
            ].join(''),
        initialize: function () {
