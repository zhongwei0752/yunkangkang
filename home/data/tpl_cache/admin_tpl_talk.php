<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('admin/tpl/talk|admin/tpl/header|admin/tpl/side|admin/tpl/footer|template/default/header|template/default/footer', '1387176516', 'admin/tpl/talk');?><?php $_TPL['menunames'] = array(
		'index' => '管理首页',
		'config' => '站点设置',
		'privacy' => '隐私设置',
		'usergroup' => '用户组',
...
<th>指定日志ID</th>
<td colspan="3">
<input type="text" name="doid" value="<?=$_GET['doid']?>" />
</td>
</tr>
...
<input type="hidden" name="ac" value="talk">
...
<form method="post" id="batchform" action="admincp.php?ac=talk">
<input type="hidden" name="formhash" value="<?php echo formhash(); ?>" />
...
<tr>
<td>
<input type="<?php if($allowbatch) { ?>checkbox<?php } else { ?>radio<?php } ?>" name="ids[]" value="<?=$value['doid']?>">
</td>
<td>
<a href="space.php?uid=<?=$value['uid']?>&do=talk&id=<?=$value['doid']?>" target="_blank"><?=$value['subject']?></a>
<?php if($value['friend']) { ?>[<a href="admincp.php?ac=talk&friend=<?=$value['friend']?>"><?=$value['friend']?></a>]<?php } ?>
<p class="gray">
<a href="admincp.php?ac=talk&uid=<?=$value['uid']?>"><?=$value['username']?></a><?php if($value['postip']) { ?>(<a href="admincp.php?ac=talk&postip=<?=$value['postip']?>"><?=$value['postip']?></a>)<?php } ?>
