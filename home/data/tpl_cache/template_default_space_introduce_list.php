<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/space_introduce_list|template/default/header|template/default/footer', '1396498394', 'template/default/space_introduce_list');?><?php $_TPL['titles'] = array($newname[subject]); ?>
...
<div class="content" style="font-size:15px;">
                 ...
                 <div class="bread container_12">
                     ...
                     <a href="cp.php?ac=introduce" class="btn grid_2">
                      发布
                     </a>
                 </div>	
...
<?php if(is_array($list)) { foreach($list as $value) { ?>
                     <div class="content_list container_12">
                          ...
                          <div class="grid_2">
                             <div class="list_test ">
                                  <a href="space.php?uid=<?=$value['uid']?>&do=<?=$do?>&id=<?=$value['introduceid']?>"><h3><?=$value['subject']?></h3></a>
                                  ...
                                  <p class="action_info" style="margin-top:-4px;">
                                   <a href="space.php?uid=<?=$value['uid']?>"> <span style="color:color:#AAAAAA;">发布：<?=$_SN[$value['uid']]?></span></a>
                                    <?php if($value['viewnum']) { ?><a href="space.php?uid=<?=$value['uid']?>&do=<?=$do?>&id=<?=$value['introduceid']?>"><span style="color:color:#AAAAAA;">阅读: (<?=$value['viewnum']?>)</span></a><?php } else { ?><span style="color:color:#AAAAAA;">阅读: (0)</span><?php } ?>
                                    <?php if($value['replynum']) { ?><a href="space.php?uid=<?=$value['uid']?>&do=<?=$do?>&id=<?=$value['introduceid']?>#comment"><span style="color:color:#AAAAAA;">评论: (<?=$value['replynum']?> )</span></a><?php } else { ?><span>评论: (0)</span><?php } ?><span style="color:color:#AAAAAA;"><?php echo sgmdate('Y-m-d',$value[dateline]); ?></span>
                                  </p>
                             </div>
                          </div>
                     </div><!-- list end -->
...
window.location.href = "space.php?do=introduce&view=we&fuid="+fuid;
}
</script>
<?php } ?>
