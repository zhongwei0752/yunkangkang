<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/space_dialog_list|template/default/header|template/default/footer', '1387333678', 'template/default/space_dialog_list');?>
...
<?php if(is_array($list)) { foreach($list as $value) { ?>
   		...
   		<div class = "dialog">
   			...
   			<div class = "dia-content">
   				<ul>
   					<?php $cl = $clist[$value[num]] ?>
   					<?php if(is_array($cl)) { foreach($cl as $cval) { ?>
   						<li class="q_li">
   							
   							<div class = "con-user">
   								<?php echo avatar($cval[uid],'small'); ?>
...
jquery(document).ready(function() {
 ...
 <div id="weixin">
   ...
   <h3 style="font-size:20px;color:#44B1BA;margin-left:-10px;line-height:40px;">绑定客服</h3>
    ...
    <?php if(is_array($list1)) { foreach($list1 as $value) { ?><option value="<?=$value['uid']?>"><?=$value['name']?></option><?php } } ?><!-- <div class="list_box" style=" border:1px solid red;">
