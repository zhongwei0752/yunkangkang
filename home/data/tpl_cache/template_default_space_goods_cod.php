<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/space_goods_cod|template/default/header|template/default/footer', '1387357418', 'template/default/space_goods_cod');?><?php $_TPL['titles'] = array(货到付款内容); ?>
...
<div class="content" style="font-size:15px;">
                 ...
                 <div class="indexing">
                   <span><a href="space.php?do=home">首页</a></span>><span>货到付款列表</span>
                 </div><!-- end -->
...
<div class="content_detail_wrapper">
 <div id="nav" style="margin:20px auto;text-align:center;">
 <a href="space.php?do=goods&cod=1" style="padding-left:20px;<?php if($_GET['cod']=='1') { ?>color:#02B4AB<?php } ?>">未发货</a><a href="space.php?do=goods&cod=2" style="padding-left:20px;<?php if($_GET['cod']=='2') { ?>color:#02B4AB<?php } ?>">已发货</a>
 </div>
 		<?php if($list) { ?>
<?php if(is_array($list)) { foreach($list as $value) { ?>
                     <div class="content_list container_12">
                          ...
                          <div class="grid_2">
                             <div class="list_test ">
                                  <a href="space.php?uid=<?=$value['viewuid']?>&do=<?=$do?>&id=<?=$value['gid']?>"><h3><?php $wei = $value['more']; ?><?=$wei['subject']?></h3></a>
                                  <?php if($_GET['cod']=='1') { ?>
                                  <a href="cp.php?ac=goods&id=<?=$value['id']?>&op=delete1&uid=<?=$value['uid']?>&viewuid=<?=$value['viewuid']?>&gid=<?=$value['gid']?>"  style="float:right;font-size:10px;color:red;" id="blog_delete1_<?=$value['id']?>"  onclick="ajaxmenu(event, this.id)">确认收货</a>
                                  <?php } ?>
                                  <p>客户名：<?=$value['username']?></p><p>购买数量：<?=$value['number']?></p><p>客户电话：<?=$value['tel']?></p><p>客户地址：<?=$value['place']?></p><br/>
                                  <p class="action_info" style="margin-top:-4px;">
                                   ...
                                   <?php if($_GET['cod']=='2') { ?>
                                   <span style="color:color:#AAAAAA;"><?php echo sgmdate('Y-m-d ',$value[dateline1]); ?></span>
                                   <?php } else { ?>
                                    <span style="color:color:#AAAAAA;"><?php echo sgmdate('Y-m-d ',$value[dateline]); ?></span>
                                    <?php } ?>
                                  </p>
                             </div>
                          </div>
