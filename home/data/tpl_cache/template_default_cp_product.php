<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/cp_product|template/default/header|template/default/cp_topic_menu|template/default/footer|template/default/space_topic_inc', '1387254135', 'template/default/cp_product');?><?php if(empty($_SGLOBAL['inajax'])) { ?>
...
<form method="post" action="cp.php?ac=product&op=delete&productid=<?=$productid?>">
<p>确定删除指定的栏目吗？</p>
<p class="btn_line">
...
<div class="content" style="font-size:15px;">
                 ...
                 <div class="indexing">
                   <span><a href="space.php?do=home">首页</a></span>><span><a href="space.php?do=product&view=me"><?=$newname['subject']?></a></span>><span>发布</span>
                 </div><!-- end -->
                 <div class="bread container_12">
                     <div class="bread_actived grid_1">
                         发布
                     </div>
                     <a href="space.php?do=product&view=me" class="link_back_bread grid_3">
                      <?=$newname['subject']?>
                     </a>
                 </div>
<?php } ?>
...
<form method="post" action="cp.php?ac=product&productid=<?=$product['productid']?>" enctype="multipart/form-data">
...
<?php if($value['classid'] == $product['classid']) { ?>
<option value="<?=$value['classid']?>" selected><?=$value['classname']?></a></option>
<?php } else { ?>
<option value="<?=$value['classid']?>"><?=$value['classname']?></option>
<?php } ?>
<?php } } ?>
<?php if(!$product['uid'] || $product['uid']==$_SGLOBAL['supe_uid']) { ?><option value="addoption" style="color:red;">+新建分类</option><?php } ?>
</select>
                      </div>
<div class="post_list container_12">
                         <span class="select_title grid_1">标题&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                           <input type="text" class="t_input" id="subject" name="subject" size="40" value="<?=$product['subject']?>"  onblur="relatekw();" placeholder="标题长度不超过80字符" />
                            </div>
                            <div class="post_list container_12">
                         <span class="select_title grid_1">价格&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                           <input type="text" class="t_input"  id="money" size="40" name="money" value="<?=$product['money']?>">
                            </div>
                            <div class="post_list container_12">
                         <span class="select_title grid_1">封面图片&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                          <input type="file" name="files"  value="<?=$product['image1url']?>"/>
                            </div>
</td>
</tr>
<tr>
...
<div class="post_list container_12">
  ...
  <textarea class="userData" name="message" id="uchome-ttHtmlEditor" style="height:100%;width:100%;display:none;border:0px"><?=$product['message']?></textarea>
...
<input type="hidden" name="productsubmit" value="true" />
<input type="button" id="productbutton" name="productbutton" value="提交发布" onclick="validate(this);" style="display: none;" />
...
<?php if(!$_SGLOBAL['inajax'] && (!$product['uid'] || $product['uid']==$_SGLOBAL['supe_uid'])) { ?>
