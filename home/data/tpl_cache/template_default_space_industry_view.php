<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/space_industry_view|template/default/header|template/default/space_menu|template/default/space_comment_li|template/default/footer', '1387262936', 'template/default/space_industry_view');?><?php $_TPL['titles'] = array($industry['subject'], $newname[subject]); ?>
...
<?php $_TPL['spacetitle'] = " $newname[subject]";
	...
	$_TPL['spacemenus'][] = "<a href=\"space.php?uid=$space[uid]&do=industry&id=$industry[industryid]\">查看 $newname[subject]</a>"; ?>
...
<div style="padding:0 0 10px;">
            ...
            <div class="content" style="font-size:15px;">
                ...
                <span><a href="space.php?do=home">首页</a></span>><span><a href="space.php?do=industry&view=me"> <?=$newname['subject']?></a></span>
                 </div>
                 <div class="content_detail_wrapper">
                      <div class="content_page_detail">
                           <div class="content_title"><?=$industry['subject']?></div>
                           <div class="content_text_detail" style="text-align: center;margin-bottom:10px;margin-top:10px;font-size:12px;">作者 :<?=$_SN[$industry['uid']]?>&nbsp;|&nbsp;发布时间 : <?php echo sgmdate('Y-m-d H:i:s',$industry[dateline]); ?></div>
                           <div class="content_text_detail"style="overflow:hidden">
                               <p><?=$industry['message']?></p>
                           </div>
                           <div class="feed_action">
                              <ul>
                                 <li>阅览（<?=$industry['viewnum']?>）</li>
                                 <li>评论（<?=$industry['replynum']?>）</li>
                                 <?php if($_SGLOBAL['supe_uid'] == $industry['uid'] || checkperm('manageindustry')) { ?>
                                 <li><a href="cp.php?ac=industry&industryid=<?=$industry['industryid']?>&op=edit">修改</a></li>
                                 <li><a href="cp.php?ac=industry&industryid=<?=$industry['industryid']?>&op=delete" id="blog_delete_<?=$industry['industryid']?>" onclick="ajaxmenu(event, this.id)">删除</a></li>
                                 <?php } ?>
                              </ul>
                           </div>
...
<tr>
<td>
<input type="hidden" name="refer" value="space.php?uid=<?=$industry['uid']?>&do=<?=$do?>&id=<?=$id?>" />
<input type="hidden" name="id" value="<?=$id?>">
<input type="hidden" name="idtype" value="industryid">
...
<?php if($cid) { ?>
<div class="notice">
当前只显示与你操作相关的单个评论，<a href="space.php?uid=<?=$industry['uid']?>&do=industry&id=<?=$industry['industryid']?>">点击此处查看全部评论</a>
</div>
<?php } ?>
<ul id="comment_ul">
...
resizeImg('industry_article','700');
