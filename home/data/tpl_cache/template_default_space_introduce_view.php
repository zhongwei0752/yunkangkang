<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/space_introduce_view|template/default/header|template/default/space_menu|template/default/space_comment_li|template/default/footer', '1387354511', 'template/default/space_introduce_view');?><?php $_TPL['titles'] = array($introduce['subject'], $value[subject]); ?>
...
<?php } else { ?>
<?php $_TPL['spacetitle'] = "$value[subject]";
	$_TPL['spacemenus'][] = "<a href=\"space.php?uid=$space[uid]&do=$do&view=me\">TA的所有$value[subject]</a>";
	$_TPL['spacemenus'][] = "<a href=\"space.php?uid=$space[uid]&do=introduce&id=$introduce[introduceid]\">查看$value[subject]</a>"; ?>
...
<div  style="padding:0 0 10px;">
            ...
            <div class="content" style="font-size:15px;">
            	<div class="indexing" style="margin-bottom:15px;">
                  <span><a href="space.php?do=home">首页</a></span>><span><a href="space.php?do=introduce&view=me"><?=$newname['subject']?></a></span>
                 </div>
                 <div class="content_detail_wrapper">
                      <div class="content_page_detail">
                           <div class="content_title"><?=$introduce['subject']?></div>
                           <div class="content_text_detail" style="text-align: center;margin-bottom:10px;margin-top:10px;font-size:12px;">作者 : <?=$_SN[$introduce['uid']]?>&nbsp;|&nbsp;发布时间 : <?php echo sgmdate('Y-m-d H:i:s',$introduce[dateline]); ?></div>
                           <div class="content_text_detail"style="overflow:hidden">
                               <p><?=$introduce['message']?></p>
                           </div>
                           <div class="feed_action">
                              <ul>
                                 <li>阅览（<?=$introduce['viewnum']?>）</li>
                                 <li>评论（<?=$introduce['replynum']?>）</li>
                                 <?php if($_SGLOBAL['supe_uid'] == $introduce['uid'] || checkperm('manageintroduce')) { ?>
                                 <li><a href="cp.php?ac=introduce&introduceid=<?=$introduce['introduceid']?>&op=edit">修改</a></li>
                                 <li><a href="cp.php?ac=introduce&introduceid=<?=$introduce['introduceid']?>&op=delete" id="blog_delete_<?=$introduce['introduceid']?>" onclick="ajaxmenu(event, this.id)">删除</a></li>
                                 <?php } ?>
                              </ul>
                           </div>
...
<tr>
<td>
<input type="hidden" name="refer" value="space.php?uid=<?=$introduce['uid']?>&do=<?=$do?>&id=<?=$id?>" />
<input type="hidden" name="id" value="<?=$id?>">
<input type="hidden" name="idtype" value="introduceid">
...
<?php if($cid) { ?>
<div class="notice">
当前只显示与你操作相关的单个评论，<a href="space.php?uid=<?=$introduce['uid']?>&do=introduce&id=<?=$introduce['introduceid']?>">点击此处查看全部评论</a>
</div>
<?php } ?>
<ul id="comment_ul">
