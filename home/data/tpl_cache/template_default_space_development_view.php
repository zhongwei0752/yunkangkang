<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/space_development_view|template/default/header|template/default/space_menu|template/default/space_comment_li|template/default/footer', '1387350651', 'template/default/space_development_view');?><?php $_TPL['titles'] = array($development['subject'], $newname[subject]); ?>
...
<?php $_TPL['spacetitle'] = "企业介绍";
	...
	$_TPL['spacemenus'][] = "<a href=\"space.php?uid=$space[uid]&do=development&id=$development[developmentid]\">查看企业介绍</a>"; ?>
                 ...
                 <div class="content_detail_wrapper">
                      <div class="content_page_detail">
                           ...
                           <div class="feed_action">
                              <ul>
                                 <li>阅览（<?=$development['viewnum']?>）</li>
                                 <li>评论（<?=$development['replynum']?>）</li>
                                 <?php if($_SGLOBAL['supe_uid'] == $development['uid'] || checkperm('managedevelopment')) { ?>
                                 <li><a href="cp.php?ac=development&developmentid=<?=$development['developmentid']?>&op=edit">修改</a></li>
                                 <li><a href="cp.php?ac=development&developmentid=<?=$development['developmentid']?>&op=delete" id="blog_delete_<?=$development['developmentid']?>" onclick="ajaxmenu(event, this.id)">删除</a></li>
                                 <?php } ?>
                              </ul>
                           </div>
...
<tr>
<td>
<input type="hidden" name="refer" value="space.php?uid=<?=$development['uid']?>&do=<?=$do?>&id=<?=$id?>" />
<input type="hidden" name="id" value="<?=$id?>">
<input type="hidden" name="idtype" value="developmentid">
...
<?php if($cid) { ?>
<div class="notice">
当前只显示与你操作相关的单个评论，<a href="space.php?uid=<?=$development['uid']?>&do=development&id=<?=$development['developmentid']?>">点击此处查看全部评论</a>
</div>
<?php } ?>
<ul id="comment_ul">
...
resizeImg('development_article','700');
