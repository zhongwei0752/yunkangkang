<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/space_menuset_view|template/default/header|template/default/space_menu|template/default/space_comment_li|template/default/footer', '1396507224', 'template/default/space_menuset_view');?><?php $_TPL['titles'] = array($menuset['subject'], '应用'); ?>
...
<div class="indexing" style="margin-bottom:15px;">
                  <img src="<?=$wei1['image2url']?>" /><span><a href="space.php?uid=<?=$space['uid']?>"><?=$_SN[$space['uid']]?></a></span>><span><a href="">应用介绍</a></span>
                 </div>
                 <div class="content_detail_wrapper">
                      <div class="content_page_detail">
                           <div class="content_title"><?=$menuset['subject']?></div>
                           <div class="content_text_detail" style="overflow:hidden">
                               <p><?=$menuset['message']?></p>
                           </div>
                           <?php if($_SGLOBAL['supe_uid']==1) { ?>
                           <div class="feed_action">
                              <ul>
                                 <li>阅览（<?=$menuset['viewnum']?>）</li>
                                 <li>评论（<?=$menuset['replynum']?>）</li>
                                 <?php if($_SGLOBAL['supe_uid'] == $menuset['uid'] || checkperm('managemenuset')) { ?>
                                 <li><a href="cp.php?ac=menuset&menusetid=<?=$menuset['menusetid']?>&op=edit">修改</a></li>
                                 <li><a href="cp.php?ac=menuset&menusetid=<?=$menuset['menusetid']?>&op=delete" id="blog_delete_<?=$menuset['menusetid']?>" onclick="ajaxmenu(event, this.id)">删除</a></li>
                                 <?php } ?>
                              </ul>
                           </div>
                           <?php } ?>
