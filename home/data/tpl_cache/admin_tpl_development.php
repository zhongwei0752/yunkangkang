<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('admin/tpl/development|admin/tpl/header|admin/tpl/side|admin/tpl/footer|template/default/header|template/default/footer', '1386835758', 'admin/tpl/development');?><?php $_TPL['menunames'] = array(
		'index' => '管理首页',
		'config' => '站点设置',
		'privacy' => '隐私设置',
		'usergroup' => '用户组',
...
<th>指定日志ID</th>
<td colspan="3">
<input type="text" name="developmentid" value="<?=$_GET['developmentid']?>" />
</td>
</tr>
...
<input type="hidden" name="ac" value="development">
...
<form method="post" id="batchform" action="admincp.php?ac=development">
<input type="hidden" name="formhash" value="<?php echo formhash(); ?>" />
...
<input type="hidden" name="ids[]" value="<?=$value['developmentid']?>">
<?php } } ?>
...
<tr>
<td>
<input type="<?php if($allowbatch) { ?>checkbox<?php } else { ?>radio<?php } ?>" name="ids[]" value="<?=$value['developmentid']?>">
</td>
<td>
<a href="space.php?uid=<?=$value['uid']?>&do=development&id=<?=$value['developmentid']?>" target="_blank"><?=$value['subject']?></a>
<?php if($value['friend']) { ?>[<a href="admincp.php?ac=development&friend=<?=$value['friend']?>"><?=$value['friend']?></a>]<?php } ?>
<p class="gray">
<a href="admincp.php?ac=development&uid=<?=$value['uid']?>"><?=$value['username']?></a><?php if($value['postip']) { ?>(<a href="admincp.php?ac=development&postip=<?=$value['postip']?>"><?=$value['postip']?></a>)<?php } ?>
&nbsp; <?php echo sgmdate('Y-m-d H:i',$value[dateline]); ?>
<br>热度(<?=$value['hot']?>) / 回复(<?=$value['replynum']?>) / 查看(<?=$value['viewnum']?>)
</p>
</td>
<td>
<a href="cp.php?ac=development&op=edit&developmentid=<?=$value['developmentid']?>" target="_blank">编辑</a>&nbsp;
<a href="admincp.php?ac=comment&id=<?=$value['developmentid']?>&idtype=developmentid">评论</a>
</td>
</tr>
<?php } } ?>
</table>
