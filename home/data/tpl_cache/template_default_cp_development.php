<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/cp_development|template/default/header|template/default/cp_topic_menu|template/default/footer|template/default/space_topic_inc', '1387264708', 'template/default/cp_development');?><?php if(empty($_SGLOBAL['inajax'])) { ?>
...
<form method="post" action="cp.php?ac=development&op=delete&developmentid=<?=$developmentid?>">
<p>确定删除指定的栏目吗？</p>
<p class="btn_line">
...
<a href="javascript:hideMenu();" class="float_del" title="关闭">关闭</a>
<div class="popupmenu_inner">
<form method="post" action="cp.php?ac=development&op=edithot&developmentid=<?=$developmentid?>">
<p class="btn_line">
新的热度：<input type="text" name="hot" value="<?=$development['hot']?>" size="5"> 
<input type="hidden" name="refer" value="<?=$_SGLOBAL['refer']?>" />
...
<div class="content" style="font-size:15px;">
                 ...
                 <div class="indexing">
                   <span><a href="space.php?do=home">首页</a></span>><span><a href="space.php?do=development&view=me"> <?=$newname['subject']?></a></span>><span>发布</span>
                 </div><!-- end -->
                 <div class="bread container_12">
                     <div class="bread_actived grid_1">
                         发布
                     </div>
                     <a href="space.php?do=development&view=me" class="link_back_bread grid_3">
                       <?=$newname['subject']?>
                     </a>
                 </div>
<?php } ?>
...
<form method="post" action="cp.php?ac=development&developmentid=<?=$development['developmentid']?>" enctype="multipart/form-data">
...
<?php if($value['classid'] == $development['classid']) { ?>
<option value="<?=$value['classid']?>" selected><?=$value['classname']?></a></option>
<?php } else { ?>
<option value="<?=$value['classid']?>"><?=$value['classname']?></option>
<?php } ?>
<?php } } ?>
<?php if(!$development['uid'] || $development['uid']==$_SGLOBAL['supe_uid']) { ?><option value="addoption" style="color:red;">+新建分类</option><?php } ?>
</select>
                      </div>
<div class="post_list container_12">
                         <span class="select_title grid_1">标题&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                           <input type="text" class="t_input" id="subject" name="subject" value="<?=$development['subject']?>" size="40" onblur="relatekw();" placeholder="标题长度不超过80字符" />
                            </div>
                            <div class="post_list container_12">
                         <span class="select_title grid_1">摘要&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                           <input type="text" class="t_input" id="tag" size="40" name="tag" value="<?=$development['tag']?>" placeholder="摘要可自己填写，也会自动获取！"> <input type="button" name="clickbutton[]" value="自动获取" class="button" onclick="relatekw();"  >
                            </div>
                            <div class="post_list container_12">
                         <span class="select_title grid_1">封面图片&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                           <input type="file" name="files" value="<?=$development['image1url']?>" />
                            </div>
</td>
</tr>
<tr>
...
<div class="post_list container_12">
                         <span class="select_title grid_1"> <?=$newname['subject']?>&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                          <a id="doodleBox" href="magic.php?mid=doodle&showid=development_doodle&target=uchome-ttHtmlEditor&from=editor" style="display:none"></a>
  <textarea class="userData" name="message" id="uchome-ttHtmlEditor" style="height:100%;width:100%;display:none;border:0px"><?=$development['message']?></textarea>
...
<input type="hidden" name="developmentsubmit" value="true" />
<input type="button" id="developmentbutton" name="developmentbutton" value="提交发布" onclick="validate(this);" style="display: none;" />
...
<?php if(!$_SGLOBAL['inajax'] && (!$development['uid'] || $development['uid']==$_SGLOBAL['supe_uid'])) { ?>
...
<input type="button" class="btn grid_3" name="clickbutton[]" value="上传图片" style="margin-right:-20px;" class="button" style="float:left;" onclick="edit_album_show('pic')">
...
<div class="confirm_btn container_12" style="margin-left:-20px;">
                           ...
                           <input type="button" class="btn grid_2" id="issuance" onclick="document.getElementById('developmentbutton').click();" value="保存发布" class="submit" />
