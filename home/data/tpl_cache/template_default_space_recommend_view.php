<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/space_recommend_view|template/default/header|template/default/space_menu|template/default/space_comment_li|template/default/footer', '1387336641', 'template/default/space_recommend_view');?><?php $_TPL['titles'] = array($recommend['subject'], '焦点推荐'); ?>
...
<?php } else { ?>
<?php $_TPL['spacetitle'] = "焦点推荐";
	$_TPL['spacemenus'][] = "<a href=\"space.php?uid=$space[uid]&do=$do&view=me\">TA的所有焦点推荐</a>";
	$_TPL['spacemenus'][] = "<a href=\"space.php?uid=$space[uid]&do=recommend&id=$recommend[recommendid]\">查看焦点推荐</a>"; ?>
...
<div style="padding:0 0 10px;">
            ...
            <div class="content" style="font-size:15px;">
            	<div class="indexing" style="margin-bottom:15px;">
                 <span><a href="space.php?do=home">首页</a></span>><span><a href="space.php?do=recommend&view=me">焦点推荐</a></span>
                 </div>
                 <div class="content_detail_wrapper">
                      <div class="content_page_detail">
                           <div class="content_title"><?=$recommend['subject']?></div>

                           <div class="content_text_detail" style="text-align: center;margin-bottom:10px;margin-top:10px;font-size:12px;">作者 : <?=$_SN[$recommend['uid']]?>&nbsp;|&nbsp;发布时间 : <?php echo sgmdate('Y-m-d H:i:s',$recommend[dateline]); ?></div>
                           ...
                           <div class="feed_action">
                              <ul>
                                 <li>阅览（<?=$recommend['viewnum']?>）</li>
                                 <li>评论（<?=$recommend['replynum']?>）</li>
                                 <?php if($_SGLOBAL['supe_uid'] == $recommend['uid'] || checkperm('managerecommend')) { ?>
                                 <li><a href="cp.php?ac=recommend&recommendid=<?=$recommend['recommendid']?>&op=edit">修改</a></li>
                                 <li><a href="cp.php?ac=recommend&recommendid=<?=$recommend['recommendid']?>&op=delete" id="blog_delete_<?=$recommend['recommendid']?>" onclick="ajaxmenu(event, this.id)">删除</a></li>
                                 <?php } ?>
                              </ul>
                           </div>
...
<tr>
<td>
<input type="hidden" name="refer" value="space.php?uid=<?=$recommend['uid']?>&do=<?=$do?>&id=<?=$id?>" />
<input type="hidden" name="id" value="<?=$id?>">
<input type="hidden" name="idtype" value="recommendid">
...
<?php if($cid) { ?>
<div class="notice">
当前只显示与你操作相关的单个评论，<a href="space.php?uid=<?=$recommend['uid']?>&do=recommend&id=<?=$recommend['recommendid']?>">点击此处查看全部评论</a>
</div>
<?php } ?>
<ul id="comment_ul">
...
resizeImg('recommend_article','700');
