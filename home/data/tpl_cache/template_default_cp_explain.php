<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/cp_explain|template/default/header|template/default/cp_topic_menu|template/default/footer|template/default/space_topic_inc', '1396328359', 'template/default/cp_explain');?><?php if(empty($_SGLOBAL['inajax'])) { ?>
...
<form method="post" action="cp.php?ac=industry&op=delete&industryid=<?=$industryid?>">
<p>确定删除指定的栏目吗？</p>
<p class="btn_line">
...
<a href="javascript:hideMenu();" class="float_del" title="关闭">关闭</a>
<div class="popupmenu_inner">
<form method="post" action="cp.php?ac=industry&op=edithot&industryid=<?=$industryid?>">
<p class="btn_line">
新的热度：<input type="text" name="hot" value="<?=$industry['hot']?>" size="5"> 
<input type="hidden" name="refer" value="<?=$_SGLOBAL['refer']?>" />
...
<div class="content" style="font-size:15px;">
                 ...
                 <div class="indexing">
                 <span><a href="space.php?do=home">首页</a></span>><span><a href="space.php?do=industry&view=me"> <?=$newname['subject']?></a></span>><span>发布</span>
                 </div><!-- end -->
                 <div class="bread container_12">
                     <div class="bread_actived grid_1">
                         发布
                     </div>
                     <a href="space.php?do=industry&view=me" class="link_back_bread grid_3">
                       <?=$newname['subject']?>
                     </a>
                 </div>
<?php } ?>
...
<form method="post" action="cp.php?ac=industry&industryid=<?=$industry['industryid']?>" enctype="multipart/form-data">
...
<?php if($value['classid'] == $industry['classid']) { ?>
<option value="<?=$value['classid']?>" selected><?=$value['classname']?></a></option>
<?php } else { ?>
<option value="<?=$value['classid']?>"><?=$value['classname']?></option>
<?php } ?>
<?php } } ?>
<?php if(!$industry['uid'] || $industry['uid']==$_SGLOBAL['supe_uid']) { ?><option value="addoption" style="color:red;">+新建分类</option><?php } ?>
</select>
                      </div>
<div class="post_list container_12">
                         <span class="select_title grid_1">标题&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                           <input type="text" class="t_input" id="subject" name="subject" value="<?=$industry['subject']?>" size="40" onblur="relatekw();" placeholder="标题长度不超过80字符" />
                            </div>
                    <div class="post_list container_12">
                         <span class="select_title grid_1">预约时间&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                           <input type="text" class="t_input" id="subject" name="subject" value="<?=$industry['subject']?>" size="20" onblur="relatekw();" placeholder="预约时间" />至
                           <input type="text" class="t_input" id="subject" name="subject" value="<?=$industry['subject']?>" size="20" onblur="relatekw();" placeholder="预约时间" />
                            </div>
                            <div class="post_list container_12">
                    <div class="post_list container_12">
                         <span class="select_title grid_1">地点&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                           <input type="text" class="t_input" id="subject" name="subject" value="<?=$industry['subject']?>" size="40" onblur="relatekw();" placeholder="地点" />     
                            </div>
                    <div class="post_list container_12">
                         <span class="select_title grid_1">预约人数&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                           <input type="text" class="t_input" id="subject" name="subject" value="<?=$industry['subject']?>" size="40" onblur="relatekw();" placeholder="预约人数" />
                            </div>
                           <!--  <div class="post_list container_12">        
                                                    <span class="select_title grid_1">摘要&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                                                    <input type="text" class="t_input" id="tag" name="tag" size="40" value="<?=$industry['tag']?>" placeholder="摘要可自己填写，也会自动获取！"> <input type="button" name="clickbutton[]" value="自动获取" class="button" onclick="relatekw();"  ><br/>
                           </div>
                           <div class="post_list container_12">
...
<input type="hidden" name="industrysubmit" value="true" />
<input type="button" id="industrybutton" name="industrybutton" value="提交发布" onclick="validate(this);" style="display: none;" />
...
<?php if(!$_SGLOBAL['inajax'] && (!$industry['uid'] || $industry['uid']==$_SGLOBAL['supe_uid'])) { ?>
...
<div class="confirm_btn container_12" style="margin-left:-20px;">
                           ...
                           <input type="button" class="btn grid_2" id="issuance" onclick="document.getElementById('industrybutton').click();" value="保存发布" class="submit" />
