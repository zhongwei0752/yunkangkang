<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/space_job_view|template/default/header|template/default/space_menu|template/default/footer', '1387269647', 'template/default/space_job_view');?><?php $_TPL['titles'] = array($job['subject'], $newname[subject]); ?>
...
<?php $_TPL['spacetitle'] = " $newname[subject]";
	...
	$_TPL['spacemenus'][] = "<a href=\"space.php?uid=$space[uid]&do=job&id=$job[jobid]\">查看$newname[subject]</a>"; ?>
...
<div style="padding:0 0 10px;">
            <div class="content" style="font-size:15px;">
                    ...
                    <span><a href="space.php?do=home">首页</a></span>><span><a href="space.php?do=job&view=me"> <?=$newname['subject']?></a></span>
                    </div><!-- end -->
                 <div class="content_detail_wrapper">
                 	
                      <div class="content_page_detail">
                           <div class="content_title"><?=$job['subject']?></div>
                         <div class="content_text_detail" style="text-align: center;margin-bottom:10px;margin-top:10px;font-size:12px;">作者 : <?=$_SN[$job['uid']]?>&nbsp;|&nbsp;发布时间 : <?php echo sgmdate('Y-m-d H:i:s',$job[dateline]); ?></div>
                           <div class="advertise_content_text_detail" style="overflow:hidden">
                           	   <?php if($job['message']) { ?>
                               <p><b>职位描述:</b><?=$job['message']?></p>
                               <?php } ?>
   <?php if($job['messagecomment']) { ?>
                               <p><b>资格要求:</b><?=$job['messagecomment']?></p>
                               <?php } ?>
                               <?php if($job['telephone']) { ?>
   <p><b>电话:</b><?=$job['telephone']?></p>
   <?php } ?>
   <?php if($job['email']) { ?>
   <p><b>邮箱:</b><?=$job['email']?></p>
   <?php } ?>
   <?php if($job['money']) { ?>
   <p><b>待遇:</b><?=$job['money']?></p>
   <?php } ?>
   <?php if($job['othermessage']) { ?>
   <p><b>其它:</b><?=$job['othermessage']?></p>
   <?php } ?>
                           ...
                           <div class="feed_action">
                              <ul>
                                 <li>阅览（<?=$job['viewnum']?>）</li>
                                 <?php if($_SGLOBAL['supe_uid'] == $job['uid'] || checkperm('managejob')) { ?>
                                 <li><a href="cp.php?ac=job&jobid=<?=$job['jobid']?>&op=edit">修改</a></li>
                                 <li><a href="cp.php?ac=job&jobid=<?=$job['jobid']?>&op=delete" id="blog_delete_<?=$job['jobid']?>" onclick="ajaxmenu(event, this.id)">删除</a></li>
                                 <?php } ?>
                              </ul>
                           </div>
