<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/cp_explaining|template/default/header|template/default/cp_topic_menu|template/default/footer|template/default/space_topic_inc', '1396577550', 'template/default/cp_explaining');?><?php if(empty($_SGLOBAL['inajax'])) { ?>
...
<form method="post" action="cp.php?ac=explaining&op=delete&explainingid=<?=$explainingid?>">
<p>确定删除指定的栏目吗？</p>
<p class="btn_line">
...
<a href="javascript:hideMenu();" class="float_del" title="关闭">关闭</a>
<div class="popupmenu_inner">
<form method="post" action="cp.php?ac=explaining&op=edithot&explainingid=<?=$explainingid?>">
<p class="btn_line">
新的热度：<input type="text" name="hot" value="<?=$explaining['hot']?>" size="5"> 
<input type="hidden" name="refer" value="<?=$_SGLOBAL['refer']?>" />
...
<div class="content" style="font-size:15px;">
                 ...
                 <div class="indexing">
                 <span><a href="space.php?do=home">首页</a></span>><span><a href="space.php?do=explaining&view=me"> <?=$newname['subject']?></a></span>><span>发布</span>
                 </div><!-- end -->
                 <div class="bread container_12">
                     <div class="bread_actived grid_1">
                         发布
                     </div>
                     <a href="space.php?do=explaining&view=me" class="link_back_bread grid_3">
                       <?=$newname['subject']?>
                     </a>
                 </div>
<?php } ?>
...
<form method="post" action="cp.php?ac=explaining&explainingid=<?=$explaining['explainingid']?>" enctype="multipart/form-data">
...
<?php if($value['classid'] == $explaining['classid']) { ?>
<option value="<?=$value['classid']?>" selected><?=$value['classname']?></a></option>
<?php } else { ?>
<option value="<?=$value['classid']?>"><?=$value['classname']?></option>
<?php } ?>
<?php } } ?>
<?php if(!$explaining['uid'] || $explaining['uid']==$_SGLOBAL['supe_uid']) { ?><option value="addoption" style="color:red;">+新建分类</option><?php } ?>
</select>
                      </div> -->
<div class="post_list container_12">
                         <span class="select_title grid_1">标题&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                           <input type="text" class="t_input" id="subject" name="subject" value="<?=$explaining['subject']?>" size="40" onblur="relatekw();" placeholder="标题长度不超过80字符" />
                            </div>
                    <div class="post_list container_12">
                        <!--  <span class="select_title grid_1">预约时间&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                          <input type="text" class="t_input" id="starttime" name="starttime" value="<?=$explaining['starttime']?>" size="20" onblur="relatekw();" placeholder="预约时间" />至
                          <input type="text" class="t_input" id="endtime" name="endtime" value="<?=$explaining['endtime']?>" size="20" onblur="relatekw();" placeholder="预约时间" /> -->
                          <span class="select_title grid_1">预约时间&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                          <script type="text/javascript" src="source/script_calendar.js" charset="<?=$_SC['charset']?>"></script>
<input type="text" class="t_input" name="starttime" id="starttime" value="<?php echo sgmdate('Y-m-d H:i', $explaining[starttime]) ?>"  onclick="showcalendar(event,this,1,'<?php echo sgmdate('Y-m-d H:i', $_SGLOBAL[timestamp]) ?>', '<?php echo sgmdate('Y-m-d H:i', $_SGLOBAL[timestamp] + 3600 * 24 * 60) ?>')" />
    至
<input type="text" class="t_input" name="endtime" id="endtime" value="<?php echo sgmdate('Y-m-d H:i', $explaining[endtime]) ?>"  onclick="showcalendar(event,this,1,'<?php echo sgmdate('Y-m-d H:i', $_SGLOBAL[timestamp]) ?>', '<?php echo sgmdate('Y-m-d H:i', $_SGLOBAL[timestamp] + 3600 * 24 * 60) ?>')" />
                            </div>
                            <div class="post_list container_12">
                    <div class="post_list container_12">
                         <span class="select_title grid_1">地点&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                           <input type="text" class="t_input" id="location" name="location" value="<?=$explaining['location']?>" size="40" onblur="relatekw();" placeholder="地点" />     
                            </div>
                    <div class="post_list container_12">
                         <span class="select_title grid_1">预约人数&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                           <input type="text" class="t_input" id="people" name="people" value="<?=$explaining['people']?>" size="40" onblur="relatekw();" placeholder="报读人数" />
                            </div>
                    <div class="post_list container_12">
                         <span class="select_title grid_1">医生人数&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                           <input type="text" class="t_input" id="doctor" name="doctor" value="<?=$explaining['doctor']?>" size="40" onblur="relatekw();" placeholder="医生人数" />
                            </div>
                           <!--  <div class="post_list container_12">        
                                                    <span class="select_title grid_1">摘要&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                                                    <input type="text" class="t_input" id="tag" name="tag" size="40" value="<?=$explaining['tag']?>" placeholder="摘要可自己填写，也会自动获取！"> <input type="button" name="clickbutton[]" value="自动获取" class="button" onclick="relatekw();"  ><br/>
                           </div>
                           <div class="post_list container_12">
                                                    <span class="select_title grid_1">封面图片&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                                                     <input type="file" name="files"  value="<?=$explaining['image1url']?>"/>
                           </div> -->
</td>
...
<td>
 <div class="post_list container_12">
                         <span class="select_title grid_1">预约详情<?=$newname['subject']?>&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                          <a id="doodleBox" href="magic.php?mid=doodle&showid=explaining_doodle&target=uchome-ttHtmlEditor&from=editor" style="display:none"></a>
  <textarea class="userData" name="message" id="uchome-ttHtmlEditor" style="height:100%;width:100%;display:none;border:0px"><?=$explaining['message']?></textarea>
...
<input type="hidden" name="explainingsubmit" value="true" />
<input type="button" id="explainingbutton" name="explainingbutton" value="提交发布" onclick="validate(this);" style="display: none;" />
...
<?php if(!$_SGLOBAL['inajax'] && (!$explaining['uid'] || $explaining['uid']==$_SGLOBAL['supe_uid'])) { ?>
...
<div class="confirm_btn container_12" style="margin-left:-20px;">
                           ...
                           <input type="button" class="btn grid_2" id="issuance" onclick="document.getElementById('explainingbutton').click();" value="保存发布" class="submit" />
