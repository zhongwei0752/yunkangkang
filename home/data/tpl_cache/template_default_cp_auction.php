<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/cp_auction|template/default/header|template/default/cp_topic_menu|template/default/footer|template/default/space_topic_inc', '1386853815', 'template/default/cp_auction');?><?php if(empty($_SGLOBAL['inajax'])) { ?>
...
<form method="post" action="cp.php?ac=auction&op=delete&auctionid=<?=$auctionid?>">
<p>确定删除指定的栏目吗？</p>
<p class="btn_line">
...
<a href="javascript:hideMenu();" class="float_del" title="关闭">关闭</a>
<div class="popupmenu_inner">
<form method="post" action="cp.php?ac=auction&op=edithot&auctionid=<?=$auctionid?>">
<p class="btn_line">
新的热度：<input type="text" name="hot" value="<?=$auction['hot']?>" size="5"> 
<input type="hidden" name="refer" value="<?=$_SGLOBAL['refer']?>" />
...
<div class="content" style="font-size:15px;">
                 ...
                 <div class="indexing">
                 <span><a href="space.php?do=home">首页</a></span>><span><a href="space.php?do=auction&view=me"> <?=$newname['subject']?></a></span>><span>发布</span>
                 </div><!-- end -->
                 <div class="bread container_12">
                     <div class="bread_actived grid_1">
                         发布
                     </div>
                     <a href="space.php?do=auction&view=me" class="link_back_bread grid_3">
                       <?=$newname['subject']?>
                     </a>
                 </div>
<?php } ?>
...
<form method="post" id="edit_form" name="edit_form" action="cp.php?ac=auction&auctionid=<?=$auction['auctionid']?>" enctype="multipart/form-data">
<table cellspacing="4" cellpadding="4" width="100%" class="infotable">
<tr>
<td>
<div class="post_list container_12">
                         <span class="select_title grid_1">拍卖物品名称&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                           <input type="text" class="t_input" id="subject" name="subject" value="<?=$auction['subject']?>" size="40" onblur="relatekw();" placeholder="标题长度不超过80字符" />
                            </div>
                            <div class="post_list container_12">
                         <span class="select_title grid_1">封面图片&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                          <input type="file" name="files"  value="<?=$auction['image1url']?>"/>
                            </div>
                            <div class="post_list container_12">
                         	<span class="select_title grid_1">开始时间&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                            <script type="text/javascript" src="source/script_calendar.js" charset="<?=$_SC['charset']?>"></script>
<input type="text" name="starttime" id="starttime" value="<?php echo sgmdate('Y-m-d H:i', $auction[starttime]) ?>"  onclick="showcalendar(event,this,1,'<?php echo sgmdate('Y-m-d H:i', $_SGLOBAL[timestamp]) ?>', '<?php echo sgmdate('Y-m-d H:i', $_SGLOBAL[timestamp] + 3600 * 24 * 60) ?>')" />
</div>
 <div class="post_list container_12">
                         	<span class="select_title grid_1">截止时间&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                            <script type="text/javascript" src="source/script_calendar.js" charset="<?=$_SC['charset']?>"></script>
<input type="text" name="endtime" id="endtime" value="<?php echo sgmdate('Y-m-d H:i', $auction[endtime]) ?>"  onclick="showcalendar(event,this,1,'<?php echo sgmdate('Y-m-d H:i', $_SGLOBAL[timestamp]) ?>', '<?php echo sgmdate('Y-m-d H:i', $_SGLOBAL[timestamp] + 3600 * 24 * 60) ?>')" />
</div>
<div class="post_list container_12">
                         <span class="select_title grid_1">起拍价&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                           <input type="text" class="t_input" id="fristprice" name="fristprice" value="<?=$auction['fristprice']?>" size="20" onblur="relatekw();" placeholder="起拍价不能为负值" />&nbsp;元
                            </div>
                            <div class="post_list container_12">
                         <span class="select_title grid_1">加价幅度&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                           <input type="text" class="t_input" id="plusprice" name="plusprice" value="<?=$auction['plusprice']?>" size="20" onblur="relatekw();" placeholder="每次加价的值" />&nbsp;元
                            </div>
                            <div class="post_list container_12">
                         <span class="select_title grid_1">管理员手机查看密码&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                           <input type="text" class="t_input" id="auctionpassword" name="auctionpassword" value="<?=$auction['auctionpassword']?>" size="20" onblur="relatekw();" placeholder="不能是空值" />&nbsp;
                            </div>
</td>
...
<td>
 <div class="post_list container_12">
                         <span class="select_title grid_1"> 描述&nbsp;&nbsp;:&nbsp;&nbsp;</span>
                          <a id="doodleBox" href="magic.php?mid=doodle&showid=auction_doodle&target=uchome-ttHtmlEditor&from=editor" style="display:none"></a>
  <textarea class="userData" name="message" id="uchome-ttHtmlEditor" style="height:100%;width:100%;display:none;border:0px"><?=$auction['message']?></textarea>
...
<input type="hidden" name="auctionsubmit" value="true" />
<input type="button" id="auctionbutton" name="auctionbutton" value="提交发布" onclick="validate(this);" style="display: none;" />
...
<?php if(!$_SGLOBAL['inajax'] && (!$auction['uid'] || $auction['uid']==$_SGLOBAL['supe_uid'])) { ?>
...
<div class="confirm_btn container_12" style="margin-left:-20px;">
                           ...
                           <input type="button" class="btn grid_2" id="issuance" onclick="document.getElementById('auctionbutton').click();" value="保存发布" class="submit" />
...
function check_auctionpost(this){
