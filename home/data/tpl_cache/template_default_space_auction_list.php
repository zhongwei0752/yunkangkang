<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/space_auction_list|template/default/header|template/default/footer', '1387333649', 'template/default/space_auction_list');?><?php $_TPL['titles'] = array($newname[subject]); ?>
...
<a href="cp.php?ac=auction" class="r_option" target="_blank">发布</a>
</h3>
<?php if($count) { ?>
<ul class="line_list">
...
<h4><a href="space.php?uid=<?=$space['uid']?>&do=auction&id=<?=$value['auctionid']?>" target="_blank" <?php if($value['magiccolor']) { ?> class="magiccolor<?=$value['magiccolor']?>"<?php } ?>><?=$value['subject']?></a></h4>
<div class="detail">
<?=$value['message']?>
</div>
</li>
...
<div class="content" style="font-size:15px;">
                 ...
                 <div class="bread container_12">
                     ...
                     <a href="cp.php?ac=auction" class="btn grid_2">
                      发布
                     </a>
                 </div>		
...
<?php if(is_array($list)) { foreach($list as $value) { ?>
                     <div class="content_list container_12">
                          ...
                          <div class="grid_2">
                             <div class="list_test ">
                             <span class="r_option"><?php if($_SGLOBAL['timestamp']<$value['starttime']) { ?>
                                  拍卖未开始
                                  <?php } ?>
                                  <?php if($_SGLOBAL['timestamp']>$value['starttime']&&$_SGLOBAL['timestamp']<$value['endtime']) { ?>
                                  拍卖进行中
                                  <?php } ?>
                                  <?php if($_SGLOBAL['timestamp']>$value['endtime']) { ?>
                                  拍卖已结束
                                  <?php } ?></span>
                                  <a href="space.php?uid=<?=$value['uid']?>&do=<?=$do?>&id=<?=$value['auctionid']?>"><h3><?=$value['subject']?></h3></a>
                                  ...
                                  <p class="action_info" style="margin-top:-4px;">
                                   <a href="space.php?uid=<?=$value['uid']?>"> <span>发布：<?=$_SN[$value['uid']]?></span></a>
                                    <?php if($value['viewnum']) { ?><a href="space.php?uid=<?=$value['uid']?>&do=<?=$do?>&id=<?=$value['auctionid']?>"><span>阅读: (<?=$value['viewnum']?>)</span></a><?php } else { ?><span>阅读: (0)</span><?php } ?>
                                    <?php if($value['replynum']) { ?><a href="space.php?uid=<?=$value['uid']?>&do=<?=$do?>&id=<?=$value['auctionid']?>#comment"><span>评论: (<?=$value['replynum']?> )</span></a><?php } else { ?><span>评论: (0)</span><?php } ?><span><?php echo sgmdate('Y-m-d',$value[dateline]); ?></span>
                                  </p>
                             </div>
                          </div>
                     </div><!-- list end -->
...
window.location.href = "space.php?do=auction&view=we&fuid="+fuid;
}
</script>
<?php } ?>
